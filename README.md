...
- `omx setup` installs prompts, skills, AGENTS scaffolding, `.codex/config.toml`, and (for legacy installs or older Codex without `plugin_hooks`) OMX-managed native Codex hooks in `.codex/hooks.json`
  ...
  - `omx setup` merges by default: it preserves existing `AGENTS.md` guidance while inserting or refreshing generated OMX sections between `<!-- OMX:AGENTS:START -->` / `<!-- OMX:AGENTS:END -->`, so updates never overwrite user-authored content. Use `--no-merge-agents` to opt out (previous behavior: refresh managed sections only / prompt before overwriting), or `--force` to replace `AGENTS.md` after backup. `--merge-agents` remains accepted and is now redundant except to re-enable merge alongside `--force`
