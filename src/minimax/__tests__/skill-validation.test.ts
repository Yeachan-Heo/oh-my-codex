...
describe('minimax skill validation', () => {
  ...
  it('requires passing council evidence when council is required', async () => {
    ...
    try {
      const councilDir = join(cwd, '.omx', 'independent-review', 'minimax');
      ...
      await writeFile(join(councilDir, 'summary.json'), JSON.stringify({
        ...
        blocker_count: 0,
        degraded: false,
        reviewer_surface: 'test-reviewer',
        scope: 'minimax completion gate',
        inputs: ['changed files', 'test output'],
      }));
      await writeFile(join(councilDir, 'partial-review.json'), JSON.stringify({
        schema_version: 'minimax-independent-review-v1',
        verdict: { status: 'pass' },
        blockers: [],
        blocker_count: 0,
        degraded: true,
        reviewer_surface: 'test-reviewer',
        scope: 'partial review should not pass',
      ...
      await writeFile(join(councilDir, 'fake-pass.json'), JSON.stringify({
        ...
        blocker_count: 0,
        reviewer_surface: 'test-reviewer',
        scope: 'schema-less review should not pass',
        inputs: ['changed files'],
      }));
      ...
      const invalidCouncil = await assessMinimaxCompletionState({
        ...
        verification_evidence_path: verificationStep1,
        council_required: true,
        council_artifact_path: '.omx/independent-review/minimax/invalid.md',
      }, cwd);
      ...
      const scalarCouncil = await assessMinimaxCompletionState({
        ...
        council_artifact_path: '.omx/independent-review/minimax/scalar.json',
      }, cwd);
      ...
      const fakeCouncil = await assessMinimaxCompletionState({
        ...
        council_required: true,
        council_evidence_step: 1,
        council_artifact_path: '.omx/independent-review/minimax/fake-pass.json',
      }, cwd);
      ...
      const incompleteSummary = await assessMinimaxCompletionState({
        ...
        council_required: true,
        council_evidence_step: 1,
        council_artifact_path: '.omx/independent-review/minimax/incomplete-summary.json',
      }, cwd);
      ...
      const blockedCouncil = await assessMinimaxCompletionState({
        ...
        council_required: true,
        council_evidence_step: 1,
        council_artifact_path: '.omx/independent-review/minimax/blocked.json',
      }, cwd);
      ...
      const withCouncil = await assessMinimaxCompletionState({
        ...
        council_required: true,
        council_evidence_step: 1,
        council_artifact_path: '.omx/independent-review/minimax/summary.json',
      }, cwd);
      assert.equal(withCouncil.complete, true);
      assert.equal(withCouncil.reason, 'arbiter_complete_with_evidence');

      const withPartialReview = await assessMinimaxCompletionState({
        mode: 'minimax',
        step: 1,
        arbiter_decision: 'complete',
        verification_evidence: ['tests passed'],
        ...
        council_required: true,
        council_evidence_step: 1,
        council_artifact_path: '.omx/independent-review/minimax/partial-review.json',
      }, cwd);
      assert.equal(withPartialReview.complete, false);
      assert.equal(withPartialReview.reason, 'missing_required_council_artifact');
