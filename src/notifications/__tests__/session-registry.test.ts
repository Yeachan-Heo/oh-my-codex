...
describeForBun("SessionMapping shape", () => {
...
describeForBun("JSONL format", () => {
	...
	it("can lookup mapping by messageId from parsed JSONL", () => {
		const target = createMockMapping({
		...
		const found = mappings.find(
			(m) => m.platform === "telegram" && m.messageId === "target-msg",
		);
		assert.ok(found);
	...
	it("can prune stale entries by age", () => {
		...
		const mappings = [
			createMockMapping({
				createdAt: new Date(now - maxAgeMs - 1000).toISOString(),
			}), // stale
			createMockMapping({ createdAt: new Date(now - 1000).toISOString() }), // fresh
			createMockMapping({
				createdAt: new Date(now - maxAgeMs + 60000).toISOString(),
			}), // just within
		];
...
describeForBun("session-registry module exports", () => {
...
describeForBun("session-registry lock contention behavior", () => {
