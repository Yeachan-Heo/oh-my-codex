...
describeForBun("getCurrentTmuxSession", () => {
...
describeForBun("getCurrentTmuxPaneId", () => {
	const originalTmux = process.env.TMUX;
...
describeForBun("getTeamTmuxSessions", () => {
...
describeForBun("getTeamTmuxSessions - session matching", () => {
	...
	it("returns prefixed worker sessions (omx-team-alpha-worker1)", () => {
		...
		assert.deepEqual(getTeamTmuxSessions("alpha"), [
			"omx-team-alpha-worker1",
			"omx-team-alpha-worker2",
		]);
	});
...
describeForBun("captureTmuxPane", () => {
	const originalPath = process.env.PATH;
	const originalTmux = process.env.TMUX;
	const originalPane = process.env.TMUX_PANE;
	const tmpDirs: string[] = [];
