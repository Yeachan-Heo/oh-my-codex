...
export function buildRalphAppendInstructions(
  ...
  return [
    ...
    '- When it reports `role_routing_unavailable`, do not fabricate `agent_type`; before any Ralplan-originated action that requires adapted role authority, run `omx ralplan preflight --json` and stop on `unsupported_documented_leader_proof`. Do not apply that gate to unrelated guarded install/sync, status/health checks, or runtime work; those flows retain their own safety gates. Never fake the role via a prompt label or infer authority from session/thread/pointer/transcript/cwd state.',
