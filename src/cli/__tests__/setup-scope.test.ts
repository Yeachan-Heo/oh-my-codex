...
describe("omx setup scope behavior", () => {
	...
	it("doctor respects persisted project setup scope paths", async () => {
		...
		try {
			...
			await mkdir(join(wd, ".agents", "skills", "sample-skill"), {
				recursive: true,
			});
			...
			await writeFile(
				join(wd, ".agents", "skills", "sample-skill", "SKILL.md"),
				"# skill\n",
			);
			await writeFile(
			...
			const normalizedWd = wd.replace(/^\/private/, "");
			assert.match(
				res.stdout,
				new RegExp(
					`Codex home: (?:/private)?${normalizedWd.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/\\.codex`,
				),
			);
			assert.doesNotMatch(
				res.stdout,
				new RegExp(`${home.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/\\.codex`),
			);
		} finally {
			await rm(wd, { recursive: true, force: true });
		}
	...
	it("doctor does not warn about missing project AGENTS.md for user scope", async () => {
		...
		try {
			...
			await mkdir(join(home, ".agents", "skills", "sample-skill"), {
				recursive: true,
			});
			...
			await writeFile(
				join(home, ".agents", "skills", "sample-skill", "SKILL.md"),
				"# skill\n",
			);
			await writeFile(
			...
			assert.match(
				res.stdout,
				/\[OK\] AGENTS\.md: user scope leaves project AGENTS\.md unchanged/,
			);
