...
describeForBun("parseAskArgs", () => {
	...
	it("parses --prompt prompt form", () => {
		assert.deepEqual(
			parseAskArgs(["gemini", "--prompt", "brainstorm", "ideas"]),
			{
				provider: "gemini",
				prompt: "brainstorm ideas",
			},
	...
	it("parses --agent-prompt with positional task text", () => {
		assert.deepEqual(
			parseAskArgs(["claude", "--agent-prompt", "executor", "review", "this"]),
			{
				provider: "claude",
				prompt: "review this",
	...
	it("throws when --agent-prompt role is missing", () => {
		assert.throws(
			() => parseAskArgs(["claude", "--agent-prompt", "--prompt", "hello"]),
			/Missing role after --agent-prompt/,
		);
	});
});

describeForBun("omx ask", () => {
	...
	it("supports claude --print and gemini --prompt end-to-end through omx ask", async () => {
		...
		try {
			...
			const claudeRes = runOmx(
				wd,
				["ask", "claude", "--print", "claude-long-flag"],
				env,
			);
			...
			const geminiRes = runOmx(
				wd,
				["ask", "gemini", "--prompt", "gemini-long-flag"],
				env,
			);
	...
	it("injects --agent-prompt content into final prompt while keeping Original task raw", async () => {
		...
		try {
			...
			assert.match(
				artifact,
				/## Final prompt[\s\S]*Follow strict verification rules\./,
			);
