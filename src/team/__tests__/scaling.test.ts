...
import { isScalingEnabled, scaleDown, scaleUp } from "../scaling.js";
import {
	createTask,
	initTeamState,
	readTask,
...
describeForBun("isScalingEnabled", () => {
	...
	it('returns false when env var is "false"', () => {
		assert.equal(
			isScalingEnabled({ OMX_TEAM_SCALING_ENABLED: "false" }),
			false,
		);
	});
	...
	it('returns true when env var is "enabled"', () => {
		assert.equal(
			isScalingEnabled({ OMX_TEAM_SCALING_ENABLED: "enabled" }),
			true,
		);
	});
	...
	it("returns true with leading/trailing whitespace", () => {
		assert.equal(isScalingEnabled({ OMX_TEAM_SCALING_ENABLED: "  1  " }), true);
		assert.equal(
			isScalingEnabled({ OMX_TEAM_SCALING_ENABLED: " true " }),
			true,
		);
	});
});
...
describeForBun("WorkerStatus draining state", () => {
...
describeForBun("Monotonic worker index counter", () => {
	it("initTeamState sets next_worker_index to workerCount + 1", async () => {
		...
		try {
			...
			const diskCfg = JSON.parse(
				readFileSync(
					join(cwd, ".omx", "state", "team", "idx-test", "config.json"),
					"utf8",
				),
	...
	it("next_worker_index is present in manifest.v2.json", async () => {
		...
		try {
			...
			const manifest = JSON.parse(
				readFileSync(
					join(cwd, ".omx", "state", "team", "manif-test", "manifest.v2.json"),
					"utf8",
				),
			) as { next_worker_index?: number };
...
describeForBun("withScalingLock", () => {
...
describeForBun("scaleUp", () => {
	...
	it("returns error for invalid count", async () => {
		const result = await scaleUp("test", 0, "executor", [], "/tmp", {
	...
	it("returns error for negative count", async () => {
		const result = await scaleUp("test", -1, "executor", [], "/tmp", {
			OMX_TEAM_SCALING_ENABLED: "1",
		});
		assert.equal(result.ok, false);
		if (!result.ok) {
	...
	it("persists scaled-up task roles in canonical task state and inbox ids", async () => {
		...
		try {
			...
			await writeFile(
				join(cwd, ".codex", "prompts", "writer.md"),
				"<identity>You are Writer.</identity>",
			);
			await mkdir(join(cwd, ".omx", "state", "team", "scale-up-role"), {
				recursive: true,
			});
			await writeFile(
				join(cwd, ".omx", "state", "team", "scale-up-role", "worker-agents.md"),
...
`,
			...
			const result = await scaleUp("scale-up-layout", 1, "executor", [], cwd, {
				OMX_TEAM_SCALING_ENABLED: "1",
...
describeForBun("scaleDown teardown hardening", () => {
	it("scaleDown removes workers when pane is already dead or missing", async () => {
		...
		try {
			...
			assert.ok(updated);
			assert.equal(
				updated?.workers.some((worker) => worker.name === "worker-2"),
				false,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
