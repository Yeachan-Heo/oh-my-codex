...
function listPanes(target: string): TmuxPaneInfo[] {
	...
	return result.stdout
		...
		.map((line) => {
			const [paneId = "", currentCommand = "", startCommand = ""] =
				line.split("\t");
...
export function chooseTeamLeaderPaneId(
	panes: TmuxPaneInfo[],
...
async function runTmuxAsync(
	...
	try {
		...
		return {
			ok: false,
			stderr: (err.stderr || err.message || "").trim() || "tmux command failed",
		};
	}
}
...
export function buildResizeHookTarget(
	sessionName: string,
	windowIndex: string,
): string {
...
export function buildRegisterResizeHookArgs(
	...
	const resizeCommand = shellQuoteSingle(
		buildBestEffortShellCommand(
			`tmux ${buildHudResizeCommand(hudPaneId, heightLines)}`,
		),
	);
	return [
		"set-hook",
		"-t",
		hookTarget,
		buildResizeHookSlot(hookName),
		`run-shell -b ${resizeCommand}`,
	];
}

export function buildUnregisterResizeHookArgs(
	hookTarget: string,
	hookName: string,
): string[] {
...
export function buildRegisterClientAttachedReconcileArgs(
	...
	return [
		...
		hookTarget,
		hookSlot,
		`run-shell -b ${oneShotCommand}`,
	];
}

export function buildUnregisterClientAttachedReconcileArgs(
	hookTarget: string,
	hookName: string,
): string[] {
	return [
		...
		"-t",
		hookTarget,
		buildClientAttachedHookSlot(hookName),
	];
}

export function unregisterResizeHook(
	hookTarget: string,
	hookName: string,
): boolean {
...
export function buildScheduleDelayedHudResizeArgs(
	...
	return [
		"run-shell",
		"-b",
		`sleep ${delay}; ${buildBestEffortShellCommand(`tmux ${buildHudResizeCommand(hudPaneId, heightLines)}`)}`,
	];
}
...
export function buildWorkerLaunchSpec(
	existsFn: (path: string) => boolean = existsSync,
): WorkerLaunchSpec {
	if (isMsysOrGitBash()) return { shell: "/bin/sh", rcFile: null };
	const zsh = resolveShellPath(ZSH_CANDIDATE_PATHS, existsFn);
	if (zsh) return { shell: zsh, rcFile: "~/.zshrc" };
	const bash = resolveShellPath(BASH_CANDIDATE_PATHS, existsFn);
	if (bash) return { shell: bash, rcFile: "~/.bashrc" };
...
function normalizeTeamWorkerCliMode(
	...
	sourceEnv: string = OMX_TEAM_WORKER_CLI_ENV,
): TeamWorkerCliMode {
	...
	if (
		...
		return normalized;
	throw new Error(
		`Invalid ${sourceEnv} value "${raw}". Expected: auto, codex, claude, gemini`,
	);
}
...
export function resolveTeamWorkerLaunchMode(
	...
	const raw = String(env[OMX_TEAM_WORKER_LAUNCH_MODE_ENV] ?? "interactive")
		.trim()
		.toLowerCase();
	if (raw === "" || raw === "interactive") return "interactive";
	if (raw === "prompt") return "prompt";
	throw new Error(
		`Invalid ${OMX_TEAM_WORKER_LAUNCH_MODE_ENV} value "${env[OMX_TEAM_WORKER_LAUNCH_MODE_ENV]}". Expected: interactive, prompt`,
	);
}
...
export function resolveTeamWorkerCli(
	launchArgs: string[] = [],
	env: NodeJS.ProcessEnv = process.env,
): TeamWorkerCli {
...
export function resolveTeamWorkerCliPlan(
	...
	const fallbackAutoFromArgs = (): TeamWorkerCli =>
		resolveTeamWorkerCliFromLaunchArgs(launchArgs);
...
function commandExists(binary: string): boolean {
	...
	if (result.error) {
		return (
			classifySpawnError(result.error as NodeJS.ErrnoException) !== "missing"
		);
	}
	return true;
}
...
export function buildWorkerStartupCommand(
	...
	const pathPrefix = leaderNodeDir
		? `export PATH='${leaderNodeDir}':$PATH; `
		: "";
	const quotedArgs = processSpec.args.map(shellQuoteSingle).join(" ");
	const cliInvocation =
		quotedArgs.length > 0
			? `exec ${processSpec.command} ${quotedArgs}`
			: `exec ${processSpec.command}`;
	const rcPrefix = launchSpec.rcFile
		? `if [ -f ${launchSpec.rcFile} ]; then source ${launchSpec.rcFile}; fi; `
		: "";
	const inner = `${rcPrefix}${pathPrefix}${cliInvocation}`;
	const envParts = Object.entries(processSpec.env).map(
		([key, value]) => `${key}=${value}`,
	);
...
export function buildWorkerProcessLaunchSpec(
	...
	const workerCli =
		workerCliOverride ?? resolveTeamWorkerCli(fullLaunchArgs, effectiveEnv);
	const cliLaunchArgs = translateWorkerLaunchArgsForCli(
		workerCli,
		fullLaunchArgs,
		initialPrompt,
	);
	const effectiveCliLaunchArgs =
		workerCli === "codex" && !cliLaunchArgs.includes(CODEX_BYPASS_FLAG)
...
export function createTeamSession(
	...
	const normalizedWorkerLaunchArgs = resolveWorkerLaunchArgs(
		workerLaunchArgs,
		cwd,
	);
	const defaultWorkerCliPlan = resolveTeamWorkerCliPlan(
		workerCount,
		normalizedWorkerLaunchArgs,
		process.env,
	);
	const workerCliPlan =
		workerStartups.length > 0
			? workerStartups.map(
					(startup, index) => startup.workerCli ?? defaultWorkerCliPlan[index]!,
				)
	...
	let registeredClientAttachedHook: { name: string; target: string } | null =
		null;
	...
	try {
		...
		if (!context.ok) {
			...
			throw new Error(
				`failed to detect current tmux target${paneHint}: ${context.stderr}`,
			);
		}
		...
		if (omxEntry && omxEntry.trim() !== "") {
			...
			if (hudResult.ok) {
				...
				if (id.startsWith("%")) {
					...
					resizeHookName = buildResizeHookName(
						safeTeamName,
						sessionName,
						windowIndex,
						hudPaneId,
					...
					if (!registerHook.ok) {
						throw new Error(
							`failed to register resize hook ${resizeHookName}: ${registerHook.stderr}`,
						);
					}
					...
					if (!delayed.ok) {
						throw new Error(
							`failed to schedule delayed HUD resize: ${delayed.stderr}`,
						);
					}
					const reconcile = runTmux(buildReconcileHudResizeArgs(hudPaneId));
					if (!reconcile.ok) {
						throw new Error(
							`failed to reconcile HUD resize: ${reconcile.stderr}`,
						);
					}
				}
			}
...
function resolveWorkerCliFromMapForSend(
	...
	if (entries.length === 0 || entries.some((entry) => entry.length === 0))
		return null;
	const selectedRaw =
		entries.length === 1 ? entries[0] : entries[workerIndex - 1];
	if (!selectedRaw) return null;
	try {
		const mode = normalizeTeamWorkerCliMode(
			selectedRaw,
...
export function buildWorkerSubmitPlan(
	...
	const queueRequested =
		strategy === "queue" || (strategy === "auto" && paneBusyAtStart);
	return {
