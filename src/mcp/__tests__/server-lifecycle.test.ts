...
const IDLE_ENTRYPOINTS = [
	...
	{ server: "trace", file: "trace-server.js" },
	{
		...
		file: "team-server.js",
		caveat:
			"idle teardown only; request-time child-job semantics remain intentionally out of scope",
	},
] as const;

const IS_BUN_RUNTIME = "Bun" in globalThis;
const describeWithOptionalSkip =
	(describe as typeof describe & { skip?: typeof describe }).skip ?? describe;
const describeServerLifecycle = IS_BUN_RUNTIME
	? describeWithOptionalSkip
	: describe;
...
async function waitForSpawn(
	...
	await Promise.race([
		...
		delay(SPAWN_TIMEOUT_MS).then(() => {
			throw new Error(
				`timed out waiting for spawn: ${formatFailureContext(entrypoint, stderr, stdout)}`,
			);
		}),
	]);
}
	...
	try {
		const [code, signal] = (await Promise.race([
			...
			delay(EXIT_TIMEOUT_MS).then(() => {
				throw new Error(
					`timed out waiting for exit: ${formatFailureContext(entrypoint, stderr, stdout)}`,
				);
			}),
...
function spawnEntrypoint(entrypoint: EntryPoint): {
	...
	const child = spawn(
		process.execPath,
		[join(process.cwd(), "dist", "mcp", entrypoint.file)],
		{
			cwd: process.cwd(),
			env: { ...process.env },
...
describeServerLifecycle(
	"MCP stdio lifecycle runtime regression (built entrypoints)",
	() => {
		for (const entrypoint of IDLE_ENTRYPOINTS) {
			const label =
				"caveat" in entrypoint
			...
			it(label, async () => {
				...
				try {
					...
					await assertChildAliveBeforeTeardown(
						child,
						entrypoint,
						stderr,
						stdout,
