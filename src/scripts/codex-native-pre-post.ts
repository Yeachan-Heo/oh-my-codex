import {
	buildLoreCommitGuardEnvWithConfigFallback,
...
export function normalizePostToolUsePayload(
	...
	const parsedToolResponse =
		tryParseJsonString(rawToolResponse) ?? safeObject(rawToolResponse);
	const exitCode =
		safeInteger(parsedToolResponse?.exit_code) ??
...
function isMcpLikeToolName(toolName: string): boolean {
	return /^(mcp__|omx_(?:state|memory|trace|code_intel)\b|state_|project_memory_|notepad_|trace_)/i.test(
		toolName,
	);
}
...
export function detectMcpTransportFailure(
	...
	const mcpContextDetected =
		isMcpLikeToolName(normalized.toolName) ||
		/\bmcp\b/i.test(combined) ||
		/\bomx-(?:state|memory|trace|code-intel)-server\b/i.test(combined);
	if (!mcpContextDetected) return null;
	if (!combined) return null;
	if (
		!MCP_TRANSPORT_FAILURE_PATTERNS.some((pattern) => pattern.test(combined))
	) {
		return null;
	}
...
function resolveOmxParityTarget(
	toolName: string,
): { command: OmxParityCommand; tool: string } | null {
	const match = toolName.match(
		/^mcp__omx_(state|memory|trace|code_intel)__([a-z0-9_]+)$/i,
	);
...
function buildOmxParityFallbackCommand(
	payload: CodexHookPayload,
	toolName: string,
): string | null {
...
function tokenizeShellCommandWithBoundaries(
	commandText: string,
): ShellToken[] | null {
	const trimmed = commandText.trim();
	if (!trimmed) return null;
	...
	const pushCurrent = () => {
		...
		tokens.push({
			value: current,
			startsCommand: tokens.length === 0 || nextTokenStartsCommand,
		});
		current = "";
	...
	for (let index = 0; index < trimmed.length; index += 1) {
		...
		if (char === "\n" || char === ";" || char === "&" || char === "|") {
			...
			if ((char === "&" || char === "|") && trimmed[index + 1] === char)
				index += 1;
			continue;
		}
...
function findGitCommandTokenIndex(tokens: ShellToken[]): number {
	for (
		...
		commandStart = nextCommandStart(tokens, commandStart)
	) {
		...
		while (
			index < commandEnd &&
			isInlineShellEnvAssignment(tokens[index]?.value ?? "")
		) {
			index += 1;
		}
		...
		while (
			index < commandEnd &&
			isEnvExecutableToken(tokens[index]?.value ?? "")
		) {
			index += 1;
		...
		if (index < commandEnd && isGitExecutableToken(tokens[index]?.value ?? ""))
			return index;
...
function readUnsetEnvNameFromOption(
	token: string,
...
function readInlineEnvironmentAssignments(
	tokens: ShellToken[],
	gitTokenIndex: number,
): InlineEnvironmentRead {
...
function readGitRepositorySelection(
	tokens: string[],
	gitTokenIndex: number,
...
export function parseGitCommitCommand(
	commandText: string,
): GitCommitCommandParseResult {
	...
	if (
		subcommandIndex < 0 ||
		tokens[subcommandIndex]?.toLowerCase() !== "commit"
	) {
		return {
			isGitCommit: false,
			inlineEnvironment: {},
	...
	const repositorySelection = readGitRepositorySelection(
		tokens,
		gitTokenIndex,
		subcommandIndex,
	);
	const { inlineEnvironment, environmentStartsClean, unsetEnvironmentNames } =
		readInlineEnvironmentAssignments(shellTokens ?? [], gitTokenIndex);
...
function buildEffectiveLoreCommitGuardEnv(
	parsed: GitCommitCommandParseResult,
): NodeJS.ProcessEnv {
	const effectiveEnvironment: NodeJS.ProcessEnv = parsed.environmentStartsClean
		? {}
		: { ...process.env };
...
function buildGitCommitComplianceErrors(message: string | null): string[] {
	...
	if (lines[0]?.trim() === "") {
		errors.push(
			"Start the commit message with a non-empty intent-first subject line.",
		);
	}
	...
	const { bodyText, trailerLines } = splitBodyAndTrailerLines(
		lines.slice(2).join("\n"),
	);
	const hasOmxCoauthorTrailer = trailerLines.includes(OMX_COAUTHOR_TRAILER);
	const usesCompactLorePath =
		hasSubject && hasBlankSeparator && !bodyText && hasOmxCoauthorTrailer;
	...
	if (!hasOmxCoauthorTrailer) {
		errors.push(
			`Add the required co-author trailer: \`${OMX_COAUTHOR_TRAILER}\`.`,
		);
	}
...
function buildGitCommitEnforcementOutput(
	commandText: string,
	...
	if (
		!isLoreCommitGuardEnabled(
			buildLoreCommitGuardEnvWithConfigFallback(
				buildEffectiveLoreCommitGuardEnv(parsed),
			),
		)
	)
		return null;
...
function commandStartsWithReadOnlyInspection(command: string): boolean {
	...
	if (commandToken === "env") {
		const nextCommand = tokens.find(
			(token, index) =>
				index > 0 &&
				!token.startsWith("-") &&
				!isInlineShellEnvAssignment(token),
		);
		commandToken = nextCommand ?? commandToken;
	}
	const basename =
		commandToken.replace(/\\/g, "/").split("/").pop()?.toLowerCase() ??
	...
	if (
		basename === "cat" &&
		/(?:^|[;&|]\s*)cat\b[\s\S]{0,200}>\s*[^\s&|;]+/.test(command)
	)
		return false;
	return !/\|\s*(?:sh|bash|zsh|python3?|node|perl|ruby|apply_patch)\b/i.test(
		command,
	);
}
...
function commandHasWriteLikeIntent(command: string): boolean {
	return (
		...
		/(?:^|[;&|]\s*)(?:cat|printf|echo)\b[\s\S]{0,200}>\s*[^\s&|;]+/.test(
			command,
		) ||
		/\btee\s+(?:-a\s+)?[^\s&|;]+/.test(command) ||
		/\bsed\s+(?:[^\n;&|]*\s)?-i(?:\b|['"])/.test(command) ||
		/\b(?:python3?|node|perl|ruby)\b[\s\S]{0,240}\b(?:writeFileSync|writeFile|write_text|open\([^)]*["']w|File\.write|Path\()/.test(
			command,
		) ||
		/<<['"]?[A-Za-z0-9_ -]+['"]?[\s\S]*(?:^|\n)(?:\+\+\+\s|---\s|import\s|export\s|function\s|const\s|class\s|interface\s)/m.test(
			command,
		)
	);
}
...
function buildSloppyFallbackPreToolUseOutput(
	commandText: string,
): Record<string, unknown> | null {
...
function commandInvokesOmxQuestion(command: string): boolean {
	const tokens =
		tokenizeShellCommand(command)?.map((token) => token.toLowerCase()) ?? [];
	for (let index = 0; index < tokens.length; index += 1) {
		const rawToken = tokens[index] || "";
		const token = rawToken.replace(/\\/g, "/").split("/").pop() || "";
		if (
			(token === "omx" || token === "omx.js") &&
			tokens[index + 1] === "question"
		)
			return true;
		if (
			(token === "node" || token === "node.exe") &&
			/(?:^|\/)omx\.js$/.test(tokens[index + 1] || "") &&
			tokens[index + 2] === "question"
		)
			return true;
	}
	return (
		/\bomx\s+question\b/i.test(command) ||
...
function isQuestionReturnPaneAssignment(token: string): boolean {
	...
	if (
		!["OMX_QUESTION_RETURN_PANE", "OMX_LEADER_PANE_ID", "TMUX_PANE"].includes(
			name,
		)
	)
		return false;
...
function commandHasPowerShellQuestionReturnPane(command: string): boolean {
	return (
		/\$env:(?:OMX_QUESTION_RETURN_PANE|OMX_LEADER_PANE_ID)\s*=\s*(?:['"]?%\d+['"]?|\$env:TMUX_PANE)\b/i.test(
			command,
...
function commandHasQuestionReturnPane(command: string): boolean {
	...
	return (tokenizeShellCommand(command) ?? []).some(
		isQuestionReturnPaneAssignment,
	);
}
...
function commandInvokesOmxTeam(command: string): boolean {
	...
	return (
		/\bomx\s+team\b/i.test(command) || /\bomx\.js['"]?\s+team\b/i.test(command)
	);
}
...
function commandInvokesOmxHud(command: string): boolean {
	...
	return (
		/\bomx\s+hud\b/i.test(command) || /\bomx\.js['"]?\s+hud\b/i.test(command)
	);
}
...
function buildNativeOmxHudPreToolUseEnforcementOutput(
	...
	return {
		decision: "block",
		reason:
			"omx hud cannot be launched directly from Codex App/native outside-tmux Bash sessions.",
		systemMessage:
			"omx hud is blocked from Bash in Codex App/native outside-tmux sessions; use SessionStart/HUD context instead, or launch OMX CLI from an attached tmux shell first for the tmux HUD runtime.",
	};
}
...
function buildNativeOmxTeamPreToolUseEnforcementOutput(
	...
	return {
		decision: "block",
		reason:
			"omx team cannot be launched directly from Codex App/native outside-tmux Bash sessions.",
...
function buildOmxQuestionPreToolUseEnforcementOutput(
	...
	if (isNativeOutsideTmuxSurface(payload)) {
		return {
			decision: "block",
			reason:
				"omx question cannot be launched directly from Codex App/native outside-tmux Bash sessions.",
	...
	return {
		decision: "block",
		reason:
			"omx question Bash invocations must preserve the leader pane return target.",
...
export function buildNativePreToolUseOutput(
	...
	if (!normalized.isBash) return null;
	const gitCommitEnforcement = buildGitCommitEnforcementOutput(
		normalized.normalizedCommand,
	);
	if (gitCommitEnforcement) return gitCommitEnforcement;
	const hudEnforcement = buildNativeOmxHudPreToolUseEnforcementOutput(
		normalized.normalizedCommand,
		payload,
	);
	if (hudEnforcement) return hudEnforcement;
	const teamEnforcement = buildNativeOmxTeamPreToolUseEnforcementOutput(
		normalized.normalizedCommand,
		payload,
	);
	if (teamEnforcement) return teamEnforcement;
	const questionEnforcement = buildOmxQuestionPreToolUseEnforcementOutput(
		normalized.normalizedCommand,
		payload,
	);
	...
	const sloppyFallbackWarning = buildSloppyFallbackPreToolUseOutput(
		normalized.normalizedCommand,
	);
...
function containsHardFailure(text: string): boolean {
	return /command not found|permission denied|no such file or directory/i.test(
		text,
	);
}

function hasActionableBashHardFailure(
...
export function buildNativePostToolUseOutput(
	...
	const normalized = normalizePostToolUsePayload(payload);
	const mcpTransportFailure = normalized.isBash
		? null
		: detectMcpTransportFailure(payload);
	if (mcpTransportFailure) {
		const fallbackCommand = buildOmxParityFallbackCommand(
			payload,
			mcpTransportFailure.toolName,
		);
