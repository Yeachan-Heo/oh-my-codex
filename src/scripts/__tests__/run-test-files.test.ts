    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "  // ASSERTION-CHANGE-JUSTIFIED: config homes are now always isolated instead of merely deleted.",
          "  assert.equal(process.env.CODEX_HOME?.startsWith(process.env.OMX_TEST_SANDBOX), true);",
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "  assert.equal(process.env.OMX_ROOT, undefined);",
          "  // ASSERTION-CHANGE-JUSTIFIED: an ambient CODEX_HOME is replaced by the per-file sandbox.",
          "  assert.equal(process.env.CODEX_HOME?.startsWith(process.env.OMX_TEST_SANDBOX), true);",
          "});",
          '',
        ].join('\n'),
    ...
    try {
      ...
      writeFileSync(
        ...
        [
          ...
          "  // ASSERTION-CHANGE-JUSTIFIED: preserving runtime state never authorizes personal config inheritance.",
          "  assert.equal(process.env.CODEX_HOME?.startsWith(process.env.OMX_TEST_SANDBOX), true);",
          "});",
          '',
        ].join('\n'),
