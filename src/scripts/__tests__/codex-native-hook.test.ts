...
`,
  ...
  it("allows non-Lore git commit messages when the Lore commit guard is disabled in CODEX_HOME config.toml", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-config-disabled-"));
    const codexHome = await mkdtemp(join(tmpdir(), "omx-native-hook-codex-home-"));
    ...
    try {
      ...
      if (originalLoreGuard === undefined) delete process.env.OMX_LORE_COMMIT_GUARD;
  ...
  it("keeps inline Lore guard assignments higher priority than CODEX_HOME config.toml", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-inline-priority-"));
    const codexHome = await mkdtemp(join(tmpdir(), "omx-native-hook-inline-priority-codex-home-"));
    const originalLoreGuard = process.env.OMX_LORE_COMMIT_GUARD;
    const originalCodexHome = process.env.CODEX_HOME;
    try {
      delete process.env.OMX_LORE_COMMIT_GUARD;
      ...
      const result = await dispatchCodexNativeHook(
        {
          ...
          tool_input: { command: 'OMX_LORE_COMMIT_GUARD=1 git commit -m "fix: conventional commit"' },
        },
        { cwd },
      );
