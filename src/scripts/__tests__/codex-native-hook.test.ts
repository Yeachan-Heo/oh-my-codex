...
describe("codex native hook dispatch", () => {
	...
	it("does not crash Stop hook dispatch when the exec follow-up queue is malformed", async () => {
		...
		try {
			...
			assert.ok(
				queueDirEntries.some((entry) =>
					entry.startsWith("exec-followups.json.corrupt-"),
				),
			);
...
`,
	...
	it("emits deterministic Stop JSON stdout when Stop dispatch fails", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-cli-stop-dispatch-failure-"),
		);
		try {
			const stdout = runNativeHookCli(
				{
			...
			assert.match(
				...
				/failed before normal continuation handling/,
			);
			assert.match(
				String(output.systemMessage),
				/test-induced Stop dispatch failure/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("logs Stop dispatch failures without foreground stderr noise", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-cli-stop-dispatch-silent-"),
		);
		try {
			...
			assert.equal(
				logFiles.some((name) =>
					/^native-hook-\d{4}-\d{2}-\d{2}\.jsonl$/.test(name),
				),
				true,
			);
		} finally {
	...
	it("keeps non-Stop dispatch failures fail-closed without foreground stderr noise", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-cli-pretool-dispatch-silent-"),
		);
		try {
			const result = spawnSync(process.execPath, [nativeHookScriptPath()], {
				cwd,
	...
	it("maps Codex events onto OMX logical surfaces", () => {
		...
		assert.equal(
			mapCodexHookEventToOmxEvent("UserPromptSubmit"),
			"keyword-detector",
		);
	...
	it("emits no CLI stdout for PreCompact when no Codex action is needed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-precompact-cli-"),
		);
		try {
			writePage(cwd, {
				filename: "architecture.md",
	...
	it("emits no CLI stdout for PostCompact when no Codex action is needed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-postcompact-cli-"),
		);
		try {
			const stdout = runNativeHookCli({
				hook_event_name: "PostCompact",
	...
	it("writes SessionStart state against the long-lived session owner pid and injects environment context", async () => {
		...
		try {
			...
			assert.match(
				additionalContext,
				/omx team, omx hud, and omx quest(?:ion) need an attached tmux OMX CLI shell|omx team and omx hud need an attached tmux OMX CLI shell/,
			);
			assert.match(
				additionalContext,
				/not available from this outside-tmux surface/,
			);
	...
	it("preserves canonical OMX session scope when native SessionStart arrives with a different id", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-reconcile-"),
		);
		try {
	...
	it("keeps subagent SessionStart from replacing the canonical leader session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-session-start-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", canonicalSessionId, "ralph-state.json"),
				{
					active: true,
					mode: "ralph",
			...
			assert.equal(
				existsSync(
					join(stateDir, "sessions", childNativeSessionId, "ralph-state.json"),
				),
				false,
			);
			...
			assert.equal(
				tracking.sessions?.[canonicalSessionId]?.leader_thread_id,
				leaderNativeSessionId,
			);
			assert.equal(
				tracking.sessions?.[canonicalSessionId]?.threads?.[childNativeSessionId]
					?.kind,
				"subagent",
			);
			assert.equal(
			...
			assert.equal(
				tracking.sessions?.[leaderNativeSessionId]?.leader_thread_id,
				leaderNativeSessionId,
			);
			assert.equal(
				tracking.sessions?.[leaderNativeSessionId]?.threads?.[
	...
	it("does not attach a subagent SessionStart to an unrelated canonical leader", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-subagent-session-start-mismatch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("describes attached tmux runtime in SessionStart context when TMUX is present", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-start-tmux-"),
		);
		...
		try {
			...
			assert.match(
				additionalContext,
				/omx team, omx hud, and omx quest(?:ion) are directly usable in this session/,
			);
			assert.match(
				additionalContext,
	...
	it("describes direct CLI outside tmux in SessionStart context when the launch source is cli", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-start-cli-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("passes the canonical OMX session id when UserPromptSubmit revives HUD", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-hud-session-revive-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("adds .omx/ to git info/exclude during SessionStart instead of mutating repo .gitignore", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-gitignore-"),
		);
		try {
	...
	it("keeps SessionStart quiet when .omx/ is already ignored by repo-level gitignore", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-existing-ignore-"),
		);
		try {
			await writeFile(join(cwd, ".gitignore"), "node_modules/\n.omx/\n");
			execFileSync("git", ["init"], { cwd, stdio: "pipe" });
	...
	it("respects existing Git ignore resolution before writing local excludes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-global-ignore-"),
		);
	...
	it("includes persisted project-memory summary in SessionStart context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-memory-"),
		);
		try {
			await writeJson(join(cwd, ".omx", "project-memory.json"), {
				...
				directives: [
					{
						directive:
							"Keep native Stop bounded to real continuation decisions.",
						priority: "high",
					},
				],
				notes: [
	...
	it("prefers repository project-memory.json during SessionStart while preserving legacy wiki guidance", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-root-memory-legacy-wiki-"),
		);
		try {
			...
			await writeJson(join(cwd, ".omx", "project-memory.json"), {
				...
				notes: [
					{ category: "legacy", content: "stale legacy note", timestamp: now },
				],
			});
			await writeJson(join(cwd, "project-memory.json"), {
				techStack: "Canonical root memory",
				build:
					"npm run build && node --test dist/scripts/__tests__/codex-native-hook.test.js",
				conventions: "prefer repository-visible project memory at startup",
				directives: [
					{
						directive:
							"Load root project-memory.json before legacy .omx memory.",
						priority: "high",
						timestamp: now,
					},
				],
			...
			assert.doesNotMatch(
				additionalContext,
				/Legacy runtime memory should not win/,
			);
	...
	it("starts a fresh native session without inheriting stale task-scoped context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-session-isolation-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", priorSessionId, "ralph-state.json"),
				{
					active: true,
					current_phase: "executing",
				},
			...
			assert.doesNotMatch(
				additionalContext,
				/stale UI rework context snapshot/,
			);
	...
	it("records keyword activation from UserPromptSubmit payloads", async () => {
		...
		try {
			...
			assert.ok(
				result.outputJson,
				"UserPromptSubmit should emit developer context",
			);
			assert.match(
				JSON.stringify(result.outputJson),
	...
	it("warns completion-like prompts when active goal workflows need Codex snapshot reconciliation", async () => {
		...
		try {
			await writeJson(join(cwd, ".omx", "ultragoal", "goals.json"), {
				...
				goals: [
					{ id: "G001-demo", status: "in_progress", objective: "Demo goal" },
				],
			});
			...
			assert.match(
				JSON.stringify(result.outputJson),
				/requires Codex goal snapshot reconciliation/,
			);
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"My goal is to complete the migration without regressions, so I will keep testing.",
				},
				{ cwd },
			);
	...
	it("does not block ultragoal Stop after task-scoped reconciliation finishes exploded bookkeeping", async () => {
		...
		try {
			await writeJson(join(cwd, ".omx", "ultragoal", "goals.json"), {
				...
				codexObjective:
					"Complete all ultragoal stories in .omx/ultragoal/goals.json: many micro goals",
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					prompt:
						"$ralplan Architect review step. Review the draft plan and return APPROVE or ITERATE.",
				},
				{ cwd },
			);
	...
	it("keeps bare keep-going continuation on the active autopilot skill instead of denying with generic ralph overlap", async () => {
		...
		try {
			...
			assert.doesNotMatch(
				message,
				/Unsupported workflow overlap: autopilot \+ ralph\./,
			);
...
`,
	...
	it("denies direct $team prompt activation from Codex App/native outside tmux", async () => {
		...
		try {
			...
			assert.match(
				String(result.skillState?.transition_error || ""),
				/cannot activate the tmux-only `team` workflow directly/,
			);
			const message = String(
				(
					result.outputJson as {
	...
	it("surfaces transition success output for allowlisted prompt-submit handoffs", async () => {
		...
		try {
			...
			assert.match(
				JSON.stringify(result.outputJson),
				/mode transiting: deep-interview -> ralplan/,
			);
...
`,
	...
	it("blocks native/App Bash omx question with bridge-specific outside-tmux guidance", async () => {
		...
		try {
			...
			assert.equal(
				(result.outputJson as { hookSpecificOutput?: unknown } | null)
					?.hookSpecificOutput,
				undefined,
			);
			assert.match(
				String((result.outputJson as { reason?: string } | null)?.reason || ""),
				/Codex App\/native outside-tmux Bash sessions/,
			);
			assert.match(
				String(
					(result.outputJson as { systemMessage?: string } | null)
	...
	it("blocks Bash omx team from Codex App/native outside tmux", async () => {
		...
		try {
			...
			assert.match(
				String((result.outputJson as { reason?: string } | null)?.reason || ""),
				/cannot be launched directly from Codex App\/native outside-tmux Bash sessions/,
			);
			assert.match(
				String(
					(result.outputJson as { systemMessage?: string } | null)
	...
	it("blocks Bash node omx.js team from Codex App/native outside tmux", async () => {
		...
		try {
			...
			assert.match(
				...
				/Codex App\/native outside-tmux sessions/,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("returns a destructive-command caution on PreToolUse for rm -rf dist", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-danger-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent on PreToolUse for neutral pwd", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-neutral-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("warns on PreToolUse for vague sloppy fallback implementation framing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-slop-warn-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not warn on PreToolUse for read-only fallback text inspection", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-slop-readonly-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("warns when a read-only command is chained before sloppy fallback writes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-slop-chained-write-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not warn on PreToolUse for grounded compatibility fallback code", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-slop-grounded-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("keeps blocking repeated Stop while sloppy fallback diff remains", async () => {
		...
		try {
			...
			assert.equal(
				(first.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			assert.equal(
				(repeated.outputJson as { decision?: string } | null)?.decision,
				"block",
			);
			assert.equal(
				(repeated.outputJson as { stopReason?: string } | null)?.stopReason,
				"sloppy_fallback_diff_audit",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("does not block Stop when existing nearby source context grounds a new fallback line", async () => {
		const cwd = await initTempGitRepo(
			"omx-native-hook-stop-slop-existing-ground-",
		);
		try {
			await mkdir(join(cwd, "src"), { recursive: true });
			await writeFile(
	...
	it("keeps git commit Lore enforcement ahead of sloppy fallback advisory", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-slop-git-priority-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command: 'git commit -m "quick hack fallback if it fails"',
					},
				},
				{ cwd },
			);
	...
	it("blocks PreToolUse git commit with supported response shape when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-invalid-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
			...
			const hookSpecificOutput =
				(result.outputJson as { hookSpecificOutput?: Record<string, unknown> })
					.hookSpecificOutput ?? {};
	...
	it("allows non-Lore git commit messages when the Lore commit guard is explicitly disabled", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-disabled-"),
		);
	...
	it("allows non-Lore git commit messages when the Lore commit guard is disabled in CODEX_HOME config.toml", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-lore-config-disabled-",
			),
		);
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-native-hook-codex-home-"),
		);
		const originalLoreGuard = process.env.OMX_LORE_COMMIT_GUARD;
		...
		try {
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: { command: 'git commit -m "fix: conventional commit"' },
				},
				{ cwd },
			);
			...
			if (originalLoreGuard === undefined)
				delete process.env.OMX_LORE_COMMIT_GUARD;
			else process.env.OMX_LORE_COMMIT_GUARD = originalLoreGuard;
	...
	it("keeps inline Lore guard assignments higher priority than CODEX_HOME config.toml", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-lore-inline-priority-",
			),
		);
		const codexHome = await mkdtemp(
			join(tmpdir(), "omx-native-hook-inline-priority-codex-home-"),
		);
		const originalLoreGuard = process.env.OMX_LORE_COMMIT_GUARD;
		const originalCodexHome = process.env.CODEX_HOME;
		try {
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					cwd,
					tool_name: "Bash",
					tool_use_id: "tool-git-commit-lore-inline-priority",
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=1 git commit -m "fix: conventional commit"',
					},
				},
				{ cwd },
			);
	...
	it("allows non-Lore git commit messages when the Lore commit guard is disabled inline", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-lore-inline-disabled-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=0 git commit -m "fix: conventional"',
					},
				},
				{ cwd },
			);
	...
	it("does not treat newline-separated Lore guard assignment as inline git commit env", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-lore-newline-assignment-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
	...
	it("restores default-on Lore guard when env -u unsets a disabled process env", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-env-unset-"),
		);
		const original = process.env.OMX_LORE_COMMIT_GUARD;
		try {
			process.env.OMX_LORE_COMMIT_GUARD = "0";
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'env -u OMX_LORE_COMMIT_GUARD git commit -m "fix: conventional"',
					},
				},
				{ cwd },
			);
	...
	it("restores default-on Lore guard when env -i clears a disabled process env", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-env-ignore-"),
		);
		const original = process.env.OMX_LORE_COMMIT_GUARD;
		try {
			process.env.OMX_LORE_COMMIT_GUARD = "0";
	...
	it("keeps Lore commit enforcement enabled for unknown inline guard values", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-inline-unknown-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'OMX_LORE_COMMIT_GUARD=maybe git commit -m "fix: conventional"',
					},
				},
				{ cwd },
			);
	...
	it("treats Lore commit guard disabled values as trim and case tolerant", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-off-"),
		);
		const original = process.env.OMX_LORE_COMMIT_GUARD;
		try {
	...
	it("keeps Lore commit enforcement enabled for unknown guard values", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-lore-unknown-"),
		);
		const original = process.env.OMX_LORE_COMMIT_GUARD;
		try {
	...
	it("continues to later PreToolUse checks when Lore commit guard is disabled", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-lore-disabled-destructive-"),
		);
		const original = process.env.OMX_LORE_COMMIT_GUARD;
		try {
			...
			assert.match(
				JSON.stringify(result.outputJson),
				/Destructive Bash command detected/,
			);
		} finally {
			if (original === undefined) delete process.env.OMX_LORE_COMMIT_GUARD;
			else process.env.OMX_LORE_COMMIT_GUARD = original;
	...
	it("stays silent on PreToolUse for `git help commit`", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-help-commit-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent on PreToolUse for `git config alias.ci commit`", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-config-alias-commit-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent on PreToolUse for `git tag commit`", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-tag-commit-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks PreToolUse env-prefixed git commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-env-invalid-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks PreToolUse git commit when git options appear before the real commit subcommand", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-option-invalid-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command: 'git -c core.editor=true commit -m "fix tests"',
					},
				},
				{ cwd },
			);
	...
	it("blocks PreToolUse env wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-exe-commit-env-wrapper-invalid-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
	...
	it("blocks PreToolUse git.exe commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-exe-commit-invalid-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks PreToolUse env flag wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-exe-commit-env-flag-wrapper-invalid-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command: 'env -i PATH=/usr/bin git.exe commit -m "fix tests"',
					},
				},
				{ cwd },
			);
	...
	it("blocks PreToolUse env value-taking wrapper-prefixed git.exe commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-exe-commit-env-value-wrapper-invalid-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
	...
	it("blocks PreToolUse path-qualified Windows git.exe commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-exe-commit-windows-path-invalid-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command: '"C:/Program Files/Git/cmd/git.exe" commit -m "fix tests"',
					},
				},
				{ cwd },
			);
	...
	it("blocks PreToolUse quoted backslash Windows git.exe commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-exe-commit-windows-backslash-path-invalid-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: {
						command:
							'"C:\\Program Files\\Git\\cmd\\git.exe" commit -m "fix tests"',
					},
				},
				{ cwd },
			);
	...
	it("blocks PreToolUse path-qualified git commit when the inline message is not Lore-compliant", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-path-invalid-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks PreToolUse git commit when the message comes from an external source", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-file-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks PreToolUse git commit when Lore trailers exist but the OmX co-author trailer is missing", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-missing-omx-coauthor-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
	...
	it("stays silent on PreToolUse for Lore-compliant git commit with OmX co-author trailer", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-valid-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent on PreToolUse for compact inline Lore commit with only OmX co-author trailer", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-compact-coauthor-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent on PreToolUse for body-omitted inline Lore commit with decision trailers", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-git-commit-compact-trailers-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("blocks PreToolUse compact inline Lore commit when the blank separator is missing", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-pretool-git-commit-compact-no-separator-",
			),
		);
		try {
			const result = await dispatchCodexNativeHook(
	...
	it("warns on PreToolUse git commit when mapped source changes lack staged docs refresh", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-document-refresh-warn-"),
		);
		try {
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
			execFileSync("git", ["config", "user.email", "test@example.com"], {
			...
			await writeFile(
				join(cwd, "src", "scripts", "codex-native-hook.ts"),
				"export const hook = 1;\n",
				"utf-8",
			);
			...
			execFileSync(
				"git",
				["add", "README.md", "src/scripts/codex-native-hook.ts"],
				{ cwd, stdio: "ignore" },
			);
			...
			execFileSync("git", ["add", "src/scripts/codex-native-hook.ts"], {
				cwd,
				stdio: "ignore",
			});
	...
	it("does not warn on PreToolUse when relevant docs are staged", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-document-refresh-docs-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			await mkdir(join(cwd, "docs"), { recursive: true });
			...
			await writeFile(
				join(cwd, "docs", "codex-native-hooks.md"),
				"initial\n",
				"utf-8",
			);
			execFileSync(
	...
	it("does not run commit-path document-refresh against payload cwd when git -C targets another repo", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-document-refresh-chdir-"),
		);
		const otherRepo = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-document-refresh-other-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
	...
	it("suppresses PreToolUse document-refresh warning when commit message includes an exemption", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-pretool-document-refresh-exempt-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
	...
	it("returns PostToolUse remediation guidance for command-not-found output", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-failure-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						'{"exit_code":127,"stdout":"","stderr":"bash: foo: command not found"}',
				},
				{ cwd },
			);
	...
	it("stays silent when successful search output contains old Bash failure text", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-successful-search-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent when Bash stdout only contains failure-like source text", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-failure-source-text-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						"const text = 'bash: foo: command not found';\nconst detail = 'permission denied';",
				},
				{ cwd },
			);
	...
	it("stays silent for rc-zero build logs that mention missing grep paths", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-build-log-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("does not treat Bash output containing MCP transport text as MCP transport death", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-mcp-source-text-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response: JSON.stringify({
						exit_code: 0,
						stdout:
							"reason: 'MCP transport closed before response over stdio pipe closed'",
						stderr: "",
					}),
				},
				{ cwd },
	...
	it("stays silent when successful output includes prior hook context text", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-recursive-context-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	it("stays silent when successful Bash output quotes MCP transport warnings", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-bash-mcp-quote-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response: JSON.stringify({
						exit_code: 0,
						stdout:
							"diagnostic log quoted: MCP transport closed; stdio pipe closed before response",
						stderr: "",
					}),
				},
				{ cwd },
	...
	it("stays silent when Bash hard-failure text has no parsed exit code", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-bash-unparsed-failure-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						"captured transcript says: bash: foo: command not found",
				},
				{ cwd },
			);
	...
	it("returns PostToolUse MCP transport fallback guidance for clear MCP transport death", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-mcp-transport-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						'{"error":"MCP transport closed","details":"stdio pipe closed before response"}',
				},
				{ cwd },
			);
	...
	it("does not classify non-transport MCP failures as transport death", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-mcp-nontransport-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						'{"error":"validation failed","details":"mode is required"}',
				},
				{ cwd },
			);
	...
	it("marks active team state failed on MCP transport death without deleting team state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-team-mcp-transport-"),
		);
		...
		try {
			...
			assert.equal(
				existsSync(join(cwd, ".omx", "state", "team", "transport-team")),
				true,
			);
		} finally {
	...
	it("marks canonical team state failed when native payload session ids differ during MCP transport death", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-team-native-transport-"),
		);
		const previousCwd = process.cwd();
		...
		try {
			...
			const sessionState = JSON.parse(await readFile(sessionPath, "utf-8")) as {
				session_id?: string;
	...
	it("treats stderr-only informative non-zero output as reviewable instead of a generic failure", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-informative-stderr-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_response:
						'{"exit_code":8,"stdout":"","stderr":"build pending\\nlint pass"}',
				},
				{ cwd },
			);
	...
	it("treats non-zero gh pr checks style output as informative instead of a generic failure", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-informative-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
					...
					tool_input: { command: "gh pr checks" },
					tool_response:
						'{"exit_code":8,"stdout":"build\\tpending\\t2m\\nlint\\tpass\\t18s","stderr":""}',
				},
				{ cwd },
			);
	...
	it("returns MCP transport-death guidance and preserves failed team state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-mcp-dead-"),
		);
		try {
			await initTeamState(
			...
			assert.match(
				...
				/lost its transport\/server connection/,
			);
	...
	it("stays silent on neutral successful PostToolUse output", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-posttool-neutral-"),
		);
		try {
			const result = await dispatchCodexNativeHook(
				{
	...
	for (const rootActiveCase of [
		...
		it(`returns Stop continuation output from root ${rootActiveCase.mode} state when no session is active`, async () => {
			const cwd = await mkdtemp(
				join(tmpdir(), `omx-native-hook-stop-root-${rootActiveCase.mode}-`),
			);
			try {
				const stateDir = join(cwd, ".omx", "state");
	...
	it("returns Stop continuation output while Autopilot is active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("suppresses duplicate Autopilot planning Stop replays so stale planning state cannot loop indefinitely", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-planning-replay-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(
				join(stateDir, "sessions", "sess-stop-autopilot-planning-replay"),
				{ recursive: true },
			);
			await writeJson(
				join(
					stateDir,
					"sessions",
					"sess-stop-autopilot-planning-replay",
					"autopilot-state.json",
				),
				{
					active: true,
	...
	it("does not block Stop from stale root Autopilot planning state when the explicit session has no scoped state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-root-autopilot-planning-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	for (const staleRootCase of [
		...
		it(`does not block Stop from stale root ${staleRootCase.mode} state when the explicit session directory is missing`, async () => {
			const cwd = await mkdtemp(
				join(
					tmpdir(),
					`omx-native-hook-stop-missing-session-${staleRootCase.mode}-`,
				),
			);
			try {
				const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block Stop when an explicit blocked_on_user run_outcome is present on a mode state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autopilot-blocked-outcome-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(
				join(stateDir, "sessions", "sess-stop-autopilot-blocked-outcome"),
				{ recursive: true },
			);
			await writeJson(
				join(
					stateDir,
					"sessions",
					"sess-stop-autopilot-blocked-outcome",
					"autopilot-state.json",
				),
				{
					active: true,
	...
	it("returns Stop continuation output while Ultrawork is active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ultrawork-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("returns Stop continuation output while UltraQA is active", async () => {
		...
		try {
			...
			await writeJson(
				join(stateDir, "sessions", "sess-stop-ultraqa", "ultraqa-state.json"),
				{
					active: true,
	...
	it("marks leader-owned team attention during native Stop dispatch without a polling watcher", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-attention-"),
		);
		try {
			await initTeamState(
				"stop-attention-team",
			...
			const attention = await readTeamLeaderAttention(
				"stop-attention-team",
				cwd,
			);
			assert.equal(result.omxEventName, "stop");
			...
			assert.match(
				attention?.leader_session_stopped_at ?? "",
				/^\d{4}-\d{2}-\d{2}T/,
			);
			assert.deepEqual(result.outputJson, {
				decision: "block",
	...
	it("returns Stop continuation output while team phase is non-terminal", async () => {
		...
		try {
			...
			assert.deepEqual(result.outputJson, {
				...
				systemMessage:
					"OMX team pipeline is still active at phase team-verify.",
			});
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("blocks Stop for a team worker with a non-terminal assigned task via native worker context", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-"),
		);
		...
		try {
			...
			assert.deepEqual(result.outputJson, {
				...
				systemMessage:
					"OMX team worker worker-1 is still assigned task 1 (in_progress).",
			});
		} finally {
	...
	it("blocks Stop as a team-worker task failure when worker status is terminal but task evidence is not completed", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-terminal-stale-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		const prevLeaderCwd = process.env.OMX_TEAM_LEADER_CWD;
		try {
			...
			await writeJson(
				join(stateDir, "team", "worker-stale-team", "tasks", "task-1.json"),
				{
					id: "1",
					subject: "stale hook task",
					description:
						"non-completed task should still block terminal worker Stop",
					status: "in_progress",
					owner: "worker-1",
					created_at: new Date().toISOString(),
				},
	...
	it("re-blocks live team worker Stop replays but suppresses stale terminal worker repeats", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-repeat-"),
		);
		try {
			await initTeamState(
				"worker-repeat-team",
			...
			const first = await dispatchCodexNativeHook(basePayload, {
				cwd: workerCwd,
			});
			const replay = await dispatchCodexNativeHook(
			...
			const freshTurn = await dispatchCodexNativeHook(
				{
					...basePayload,
					turn_id: "turn-stop-team-worker-repeat-2",
					stop_hook_active: true,
				},
				{ cwd: workerCwd },
			);
			...
			const stateChanged = await dispatchCodexNativeHook(
				{
					...basePayload,
					turn_id: "turn-stop-team-worker-repeat-3",
					stop_hook_active: true,
				},
				{ cwd: workerCwd },
			);
			...
			assert.deepEqual(stateChanged.outputJson, {
				...
				systemMessage:
					"OMX team worker worker-1 is still assigned task 1 (blocked).",
			});
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("allows Stop for a team worker when assigned task is terminal and bypasses generic team blocking", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-terminal-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			const stopNudges =
				tmuxLog.match(
					/send-keys -t %42 -l \[OMX\] worker-1 native Stop allowed/g,
				) || [];
			assert.equal(
				stopNudges.length,
				1,
				"allowed worker Stop should nudge leader exactly once inside cooldown",
			);
			const nudgeState = JSON.parse(
				await readFile(join(workerDir, "worker-stop-nudge.json"), "utf-8"),
			);
	...
	it("allows worker Stop when the Stop nudge helper cannot deliver", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-helper-fail-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		const prevPath = process.env.PATH;
		try {
			...
			await writeFile(
				join(fakeBinDir, "tmux"),
				buildWorkerStopFakeTmux(join(cwd, "tmux.log"), { failSend: true }),
			);
			await chmod(join(fakeBinDir, "tmux"), 0o755);
			const stateDir = join(cwd, ".omx", "state");
			const workerDir = join(
			...
			await writeJson(
				join(stateDir, "team", "worker-stop-helper-fail", "config.json"),
				{
			...
			await writeJson(
				join(stateDir, "team", "worker-stop-helper-fail", "manifest.v2.json"),
				{
					name: "worker-stop-helper-fail",
					tmux_session: "omx-team-worker-stop",
					leader_pane_id: "%42",
			...
			const result = await dispatchCodexNativeHook(
				{
					hook_event_name: "Stop",
					cwd,
					session_id: "sess-stop-team-worker-helper-fail",
				},
				{ cwd },
			);
	...
	it("does not treat failed or ambiguous worker task state as completed Stop evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-failed-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		...
		try {
			...
			await writeJson(
				join(stateDir, "team", "worker-stop-failed-task", "config.json"),
				{
			...
			assert.match(
				...
				/non_completed_task_1_failed/,
			);
			assert.match(JSON.stringify(result.outputJson), /team/i);
			assert.equal(
				existsSync(join(workerDir, "worker-stop-nudge.json")),
				false,
			);
			const tmuxLog = existsSync(tmuxLogPath)
	...
	it("blocks worker Stop on missing task assignment without relying on generic team state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-missing-assignment-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevInternalTeamWorker = process.env.OMX_TEAM_INTERNAL_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
			...
			assert.equal(
				result.outputJson?.stopReason,
				"team_worker_worker-1_missing_task_assignment",
			);
			assert.equal(
				existsSync(join(workerDir, "worker-stop-nudge.json")),
				false,
	...
	it("blocks unresolved worker Stop before generic auto-nudge can bypass it", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-missing-state-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevInternalTeamWorker = process.env.OMX_TEAM_INTERNAL_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			assert.equal(
				result.outputJson?.stopReason,
				"team_worker_worker-1_missing_worker_state",
			);
	...
	it("prefers canonical internal worker identity over public worker identity for Stop nudges", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-internal-env-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevInternalTeamWorker = process.env.OMX_TEAM_INTERNAL_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			await writeJson(
				join(stateDir, "team", "internal-stop-team", "config.json"),
				{
			...
			await writeJson(
				join(stateDir, "team", "internal-stop-team", "tasks", "task-1.json"),
				{
					id: "1",
					status: "completed",
					owner: "worker-1",
			...
			assert.match(
				tmuxLog,
				/send-keys -t %42 -l \[OMX\] worker-1 native Stop allowed/,
			);
	...
	it("blocks worker Stop when canonical task ownership has a newer non-terminal task", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-worker-owned-task-"),
		);
		const prevTeamWorker = process.env.OMX_TEAM_WORKER;
		const prevInternalTeamWorker = process.env.OMX_TEAM_INTERNAL_WORKER;
		const prevTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		...
		try {
			...
			await writeJson(
				join(stateDir, "team", "worker-owned-task", "config.json"),
				{
			...
			await writeJson(
				join(stateDir, "team", "worker-owned-task", "tasks", "task-1.json"),
				{
					id: "1",
					status: "completed",
					owner: "worker-1",
			...
			await writeJson(
				join(stateDir, "team", "worker-owned-task", "tasks", "task-2.json"),
				{
					id: "2",
					status: "in_progress",
					owner: "worker-1",
			...
			assert.equal(
				result.outputJson?.stopReason,
				"team_worker_worker-1_2_in_progress",
			);
			assert.equal(
				existsSync(join(workerDir, "worker-stop-nudge.json")),
				false,
	...
	it("returns Stop continuation output from canonical team state when coarse mode state is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-canonical-"),
		);
		try {
			await initTeamState(
				"canonical-team",
	...
	it("emits one concise final decision summary and auto-finalize guidance when release-readiness already has a stable final recommendation and no active worker tasks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-release-readiness-finalize-"),
		);
		try {
			await initTeamState(
				"release-ready-team",
	...
	it("does not auto-finalize non-release team stops that happen to contain a stable recommendation summary", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-non-release-readiness-control-"),
		);
		try {
			await initTeamState(
				"general-review-team",
	...
	it("honors terminal team run-state before later canonical-team Stop fallback", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-terminal-run-state-canonical-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("re-fires canonical-team Stop output for a later fresh Stop reply when coarse mode state is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-canonical-refire-"),
		);
		try {
			await initTeamState(
	...
	it("does not block Stop from canonical team state alone when the canonical phase is terminal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-terminal-"),
		);
		try {
			await initTeamState(
				"terminal-team",
			...
			await writeJson(
				join(cwd, ".omx", "state", "team", "terminal-team", "phase.json"),
				{
					current_phase: "complete",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
	...
	it("returns Stop continuation output from canonical team state when manifest session ownership is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-legacy-"),
		);
		try {
			await initTeamState(
				"legacy-team",
	...
	it("reads canonical Stop fallback team state from OMX_TEAM_STATE_ROOT when configured", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-root-"),
		);
		...
		try {
			...
			await initTeamState(
				...
				{
					...process.env,
					OMX_SESSION_ID: "sess-stop-team-root",
					OMX_TEAM_STATE_ROOT: "shared-root",
				},
			);
			...
			assert.equal(
				existsSync(
					join(sharedRoot, "team", "canonical-root-team", "phase.json"),
				),
				true,
			);
		} finally {
	...
	it("ignores stale source-root team Stop fallback when OMX_TEAM_STATE_ROOT is authoritative", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-stale-source-root-"),
		);
		...
		try {
			process.env.OMX_TEAM_STATE_ROOT = teamStateRoot;
			await mkdir(join(cwd, ".omx", "state"), { recursive: true });
			await mkdir(join(teamStateRoot, "team", "stale-source-team"), {
				recursive: true,
			});
			await writeJson(join(cwd, ".omx", "state", "team-state.json"), {
				active: true,
			...
			await writeJson(
				join(teamStateRoot, "team", "stale-source-team", "phase.json"),
				{
					current_phase: "team-exec",
				},
			);
	...
	it("returns Stop continuation output from canonical team state rooted via OMX_TEAM_STATE_ROOT", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-env-root-"),
		);
		const previousTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
		try {
	...
	it("blocks Stop from session-scoped team mode when session.json points to another session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-session-mismatch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-live-team"), {
				recursive: true,
			});
			await writeJson(join(stateDir, "session.json"), {
				session_id: "sess-other-team",
			});
			await writeJson(
				join(stateDir, "sessions", "sess-live-team", "team-state.json"),
				{
					active: true,
					mode: "team",
					current_phase: "team-exec",
			...
			await writeJson(
				join(stateDir, "team", "session-live-team", "phase.json"),
				{
					current_phase: "team-exec",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
	...
	it("returns Stop continuation output for active ralplan skill with matching active mode state and without active subagents", async () => {
		...
		try {
			...
			await writeJson(
				join(stateDir, "sessions", "sess-stop-skill", "ralplan-state.json"),
				{
					active: true,
					current_phase: "planning",
				},
			...
			assert.match(
				String(result.outputJson?.reason ?? ""),
				/Status: continue_from_artifact/,
			);
			assert.match(
				String(result.outputJson?.reason ?? ""),
				/ralplan is still active \(phase: planning\)/,
			);
			assert.match(
				String(result.outputJson?.reason ?? ""),
				/continue from the current ralplan artifact/i,
			);
			assert.equal(
				result.outputJson?.stopReason,
	...
	it("does not block on stale ralplan skill-active state when the matching mode state is absent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-skill-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block on stale ralplan skill-active when canonical run-state is terminal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-terminal-ralplan-run-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block on stale ralplan skill-active when pinned mode state belongs to another session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-foreign-ralplan-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("returns an explicit ralplan waiting status while subagents are still active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-skill-subagent-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.match(
				String(result.outputJson?.reason ?? ""),
				/waiting for 1 active native subagent thread/,
			);
			assert.match(
				String(result.outputJson?.reason ?? ""),
				/then continue from the current ralplan artifact/i,
			);
			assert.equal(
				result.outputJson?.stopReason,
				"skill_ralplan_planning_waiting_subagent",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("does not block on stale root ralplan skill when the explicit session-scoped canonical skill state is absent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-root-skill-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("blocks Stop while autoresearch is active without validator completion", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autoresearch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				...
				),
				{
					...
					completion_artifact_path:
						".omx/specs/autoresearch-demo/completion.json",
				},
			);
			...
			assert.deepEqual(result.outputJson, {
				decision: "block",
				reason:
					"OMX autoresearch is still active (phase: executing); continue until validator evidence is complete before stopping.",
	...
	it("allows Stop once autoresearch validator evidence is complete", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-autoresearch-complete-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const specDir = join(cwd, ".omx", "specs", "autoresearch-demo");
			await mkdir(
				join(stateDir, "sessions", "sess-stop-autoresearch-complete"),
				{ recursive: true },
			);
			...
			await writeJson(
				join(
					stateDir,
					"sessions",
					"sess-stop-autoresearch-complete",
					"autoresearch-state.json",
				),
				{
					active: true,
	...
	it("does not block Stop from stale root autoresearch state when the explicit session has no scoped autoresearch state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-root-autoresearch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const specDir = join(cwd, ".omx", "specs", "autoresearch-demo");
	...
	it("does not block Stop from stale root autoresearch state when the explicit session directory is missing", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-missing-session-autoresearch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("does not block Stop solely because deep-interview is active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-deep-interview-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks Stop when deep-interview has a pending omx question obligation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-deep-interview-question-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(
				join(stateDir, "sessions", "sess-stop-deep-interview-question"),
				{ recursive: true },
			);
			await writeJson(join(stateDir, "session.json"), {
			...
			await writeJson(
				join(
					stateDir,
					"sessions",
					"sess-stop-deep-interview-question",
					"skill-active-state.json",
				),
				{
					version: 1,
	...
	it("blocks Stop when a same-session deep-interview question obligation is pending even after the mode marked itself inactive", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-deep-interview-question-inactive-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(
				join(
					stateDir,
					"sessions",
					"sess-stop-deep-interview-question-inactive",
				),
				{ recursive: true },
			);
			await writeJson(join(stateDir, "session.json"), {
	...
	it("does not re-block Stop after a same-session deep-interview question record is already answered", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-deep-interview-question-answered-"),
		);
		try {
			...
			assert.equal(
				state.question_enforcement?.question_id,
				"question-answered",
			);
	...
	it("keeps blocking pending deep-interview question Stop replays until the obligation changes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-deep-interview-question-replay-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(
				join(stateDir, "sessions", "sess-stop-deep-interview-question-replay"),
				{ recursive: true },
			);
			await writeJson(join(stateDir, "session.json"), {
			...
			await writeJson(
				join(
					stateDir,
					"sessions",
					"sess-stop-deep-interview-question-replay",
					"skill-active-state.json",
				),
				{
					version: 1,
	...
	it("does not block Stop once the deep-interview question obligation is satisfied or cleared", async () => {
		for (const status of ["satisfied", "cleared"] as const) {
			const cwd = await mkdtemp(
				join(
					tmpdir(),
					`omx-native-hook-stop-deep-interview-question-${status}-`,
				),
			);
			try {
				const stateDir = join(cwd, ".omx", "state");
				await mkdir(
					join(
						stateDir,
						"sessions",
						`sess-stop-deep-interview-question-${status}`,
					),
					{ recursive: true },
				);
				await writeJson(join(stateDir, "session.json"), {
					...
					),
					{
						...
						question_enforcement: {
							...
							...(status === "satisfied"
								...
								: {
										cleared_at: "2026-04-19T03:21:00.000Z",
										clear_reason: "error",
									}),
						},
					},
	...
	it("ignores pending deep-interview question obligations from another session", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-deep-interview-question-foreign-session-",
			),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", "sess-other", "skill-active-state.json"),
				{
					version: 1,
					active: true,
					skill: "deep-interview",
			...
			await writeJson(
				join(stateDir, "sessions", "sess-other", "deep-interview-state.json"),
				{
					active: true,
					mode: "deep-interview",
					current_phase: "intent-first",
	...
	it("blocks a new same-session deep-interview question obligation even after an earlier round was satisfied", async () => {
		const cwd = await mkdtemp(
			join(
				tmpdir(),
				"omx-native-hook-stop-deep-interview-question-next-round-",
			),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(
				join(
					stateDir,
					"sessions",
					"sess-stop-deep-interview-question-next-round",
				),
				{ recursive: true },
			);
			await writeJson(join(stateDir, "session.json"), {
	...
	it("ignores root skill-active fallback from a different thread when evaluating Stop", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-foreign-thread-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("returns a non-blocking Stop document-refresh warning before auto-nudge when Ralph is not active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-document-refresh-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
	...
	it("does not warn on ordinary non-terminal Stop attempts before auto-nudge", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-document-refresh-nonterminal-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"Continuing implementation; next I will run focused tests.",
				},
				{ cwd },
			);
	...
	it("dedupes identical Stop document-refresh warnings during active Stop-hook replays", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-document-refresh-dedupe-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
			...
			assert.match(
				JSON.stringify(first.outputJson),
				/Document-refresh warning/,
			);
			assert.equal(replay.outputJson, null);
		} finally {
	...
	it("suppresses Stop document-refresh warning when the final handoff message includes an exemption", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-document-refresh-exempt-"),
		);
		try {
			await mkdir(join(cwd, "src", "scripts"), { recursive: true });
			execFileSync("git", ["init"], { cwd, stdio: "ignore" });
	...
	it("blocks Codex App Stop when Ralph is marked complete without completion-audit evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-complete-audit-missing-"),
		);
		try {
			...
			assert.match(
				reason,
				/Ralph completion audit is missing required evidence/,
			);
			assert.match(
				reason,
				/state\.completion_audit = \{ passed: true, prompt_to_artifact_checklist: \[\.\.\.\], verification_evidence: \[\.\.\.\] \}/,
			);
			assert.match(reason, /repo-relative JSON file/);
			assert.match(
				reason,
				/Markdown artifacts and flat top-level checklist\/evidence fields are not accepted/,
			);
			assert.equal(
				result.outputJson?.stopReason,
				"ralph_completion_audit_missing_completion_audit",
			);
			const reopened = JSON.parse(await readFile(statePath, "utf-8")) as Record<
				string,
				unknown
			>;
			...
			assert.equal(
				reopened.completion_audit_missing_reason,
	...
	it("allows Codex App Stop when complete Ralph state carries checklist and verification evidence", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-ralph-complete-audit-present-"),
		);
		try {
			...
			await writeJson(
				join(cwd, ".omx", "state", "sessions", sessionId, "ralph-state.json"),
				{
					active: false,
					mode: "ralph",
					current_phase: "complete",
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					session_id: sessionId,
					last_assistant_message:
						"Done with completion audit evidence recorded.",
				},
				{ cwd },
			);
	...
	it("blocks Stop from session-scoped Ralph state when session.json points to another session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-session-mismatch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", "sess-live-ralph", "ralph-state.json"),
				{
					active: true,
					current_phase: "executing",
	...
	it("does not block Stop from stale session-scoped Ralph state that belongs to another session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-session-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "sessions", "sess-stale", "ralph-state.json"),
				{
					active: true,
	...
	it("does not block Stop from stale current-session Ralph state when session.json points to a dead owner", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-current-session-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", "sess-dead", "ralph-state.json"),
				{
					active: true,
					current_phase: "verifying",
					session_id: "sess-dead",
			...
			await writeJson(join(stateDir, "native-stop-state.json"), {
				sessions: {
					"sess-dead": {
						last_signature:
							"ralph-stop|sess-dead|thread-1|no-message|verifying",
	...
	it("does not hard-block Stop on stale session-scoped Ralph starting state after visible active modes are cleared", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-cleared-stale-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("blocks Stop on visible active session-scoped Ralph starting state and reports its path", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-visible-starting-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block Stop from another session-scoped Ralph state when an explicit session_id has no active Ralph state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-explicit-session-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-other"), {
			...
			await writeJson(
				join(stateDir, "sessions", "sess-other", "ralph-state.json"),
				{
					active: true,
					current_phase: "starting",
					session_id: "sess-other",
	...
	it("does not block a question-only pane from Ralph state owned by another Codex session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-question-pane-"),
		);
		...
		try {
			...
			await mkdir(join(stateDir, "sessions", questionSessionId), {
				recursive: true,
			});
			await writeJson(join(stateDir, "session.json"), {
			...
			await writeJson(
				join(stateDir, "sessions", questionSessionId, "ralph-state.json"),
				{
					active: true,
					mode: "ralph",
					current_phase: "executing",
	...
	it("does not block Stop when Ralph skill-active initialization points at another session", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-stale-skill-active-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", currentSessionId, "ralph-state.json"),
				{
					active: true,
					mode: "ralph",
					current_phase: "verifying",
			...
			await writeJson(
				join(stateDir, "sessions", currentSessionId, "skill-active-state.json"),
				{
					...
					initialized_state_path:
						".omx/state/sessions/sess-old-ralph/ralph-state.json",
					active_skills: [
						{
							skill: "ralph",
							phase: "verifying",
	...
	it("blocks same-session Ralph Stop continuation when ownership identifiers match", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-owned-session-"),
		);
		const previousTmuxPane = process.env.TMUX_PANE;
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("allows native verifier subagent Stop to complete while leader Ralph remains active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-subagent-verdict-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("prefers canonical run-state terminal lifecycle before stale session Ralph state during Stop", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-canonical-run-state-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("does not block Stop from root Ralph fallback when the current session has no scoped Ralph state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-root-fallback-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
	...
	it("does not block Stop when the current session Ralph state is cancelled even if stale root fallback remains", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-cancelled-session-ralph-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "sessions", "sess-current", "ralph-state.json"),
				{
					active: false,
	...
	it("does not block Stop from root Ralph fallback when an explicit session_id is present and session.json points to another worktree", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-root-fallback-cwd-mismatch-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("keeps blocking Ralph Stop replays until the active task advances", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-replay-"),
		);
		const previousOmxSessionId = process.env.OMX_SESSION_ID;
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("lets dispatcher dedupe identical native stop hook replays after Stop payload normalization", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-hook-dedupe-"),
		);
		const previousOmxSessionId = process.env.OMX_SESSION_ID;
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("preserves per-turn native stop hook delivery even when stop_hook_active remains true", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ralph-hook-refire-"),
		);
		const previousOmxSessionId = process.env.OMX_SESSION_ID;
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("bounds repeated ordinary working Stop loops with a diagnostic summary", async () => {
		...
		try {
			...
			assert.equal(
				repeated.outputJson?.stopReason,
				"ordinary_task_no_progress_guard",
			);
			assert.match(
				String(repeated.outputJson?.systemMessage),
				/no-progress guard triggered/,
			);
			assert.match(
				String(repeated.outputJson?.systemMessage),
				/diagnostic summary/,
			...
			assert.match(
				String(repeated.outputJson?.systemMessage),
				/complete, blocked, failed, or needs missing information/,
			);

			const persisted = JSON.parse(
				await readFile(
					join(cwd, ".omx", "state", "native-stop-state.json"),
					"utf-8",
				),
			) as {
				sessions: Record<
					string,
					{ ordinary_no_progress_guard?: { repeat_count?: number } }
				>;
			};
			assert.equal(
				persisted.sessions["sess-working-loop"]?.ordinary_no_progress_guard
					?.repeat_count,
				2,
			);
		} finally {
	...
	it("re-blocks duplicate native auto-nudge replays for the same Stop reply", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-once-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("re-blocks duplicate native auto-nudge replays across native/canonical session-id drift", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-session-drift-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
			...
			assert.deepEqual(Object.keys(persisted.sessions ?? {}), [
				"omx-canonical",
			]);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("dedupes native stop hook replay across owner launch SessionStart reconciliation drift", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-dispatch-session-drift-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "omx-canonical"), {
				recursive: true,
			});
			await writeHookCounterPlugin(cwd);
			process.env.OMX_SESSION_ID = "omx-canonical";
			...
			await writeJson(
				join(stateDir, "sessions", "omx-canonical", "ralph-state.json"),
				{
					active: true,
					current_phase: "executing",
					session_id: "omx-canonical",
	...
	it("re-fires native auto-nudge for a later fresh Stop reply even when stop_hook_active is true", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-refire-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("auto-continues native Stop on permission-seeking prompts", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-permission-"),
		);
		try {
			await mkdir(join(cwd, ".omx", "state"), { recursive: true });
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"Would you like me to continue with the cleanup?",
				},
				{ cwd },
			);
	...
	it('auto-continues native Stop on "if you want" permission-seeking prompts', async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-if-you-want-"),
		);
		try {
			await mkdir(join(cwd, ".omx", "state"), { recursive: true });
			...
			const result = await dispatchCodexNativeHook(
				{
					...
					last_assistant_message:
						"If you want, I can continue with the cleanup from here.",
				},
				{ cwd },
			);
	...
	it("does not auto-continue native Stop while deep-interview is waiting on an intent-first question", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-deep-interview-question-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-stop-auto-question"), {
				recursive: true,
			});
	...
	it("suppresses native auto-nudge re-fire while session-scoped deep-interview state is still active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-deep-interview-state-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-stop-auto-interview"), {
				recursive: true,
			});
	...
	it("suppresses native auto-nudge when root deep-interview mode state is active and no session is known", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-deep-interview-mode-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("treats inherited OMX_SESSION_ID as session-aware for native auto-nudge Stop checks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-env-session-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
			...
			assert.ok(
				(stopState.sessions as Record<string, unknown>)["sess-stop-auto-mode"],
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("ignores generic SESSION_ID for native auto-nudge Stop session scoping", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-generic-session-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("does not suppress native auto-nudge from stale root deep-interview mode state when the explicit session-scoped mode state is absent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-stale-root-mode-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("does not suppress native auto-nudge from stale root deep-interview skill state when the explicit session-scoped canonical skill state is absent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-stale-root-skill-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("does not suppress native auto-nudge from stale root deep-interview input lock when the explicit session-scoped canonical skill state is absent", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-stale-root-lock-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("does not suppress native auto-nudge from active root deep-interview state when the current scoped mode state is explicitly inactive", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-inactive-scoped-mode-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-stop-auto-inactive-mode"), {
				recursive: true,
			});
	...
	it("clears stale root skill-active state when current session ralplan is terminal", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-root-skill-terminal-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			const sessionId = "sess-stop-terminal-ralplan";
			...
			assert.equal(
				rootSkillState.reconciliation_reason,
				"stop_hook_session_state_terminal",
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
		}
	...
	it("preserves legitimate session-scoped ultrawork blocking while reconciling root skill-active state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-active-root-skill-session-mode-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			await writeJson(
				join(stateDir, "sessions", sessionId, "ultrawork-state.json"),
				{
					active: true,
					mode: "ultrawork",
					current_phase: "executing",
	...
	it("reconciles stale root skill-active state under OMX_ROOT boxed state", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-boxed-source-"),
		);
		const omxRoot = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-boxed-root-"),
		);
		const previousOmxRoot = process.env.OMX_ROOT;
		try {
			process.env.OMX_ROOT = omxRoot;
			...
			assert.equal(
				boxedRootSkillState.reconciliation_reason,
				"stop_hook_session_state_terminal",
			);
			assert.equal(
				existsSync(join(sourceStateDir, "skill-active-state.json")),
				false,
			);
		} finally {
	...
	it("auto-continues native Stop for permission-seeking prompts even outside OMX runtime", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-auto-nudge-plain-session-"),
		);
		try {
			await dispatchCodexNativeHook(
				{
	...
	it("re-fires team Stop output for a later fresh Stop reply while the team is still active", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-refire-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(stateDir, { recursive: true });
	...
	it("suppresses duplicate team Stop replays across native/canonical session-id drift", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-team-session-drift-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "omx-canonical"), {
			...
			await writeJson(
				join(stateDir, "sessions", "omx-canonical", "team-state.json"),
				{
					active: true,
					current_phase: "starting",
	...
	it("suppresses duplicate ultrawork Stop replays while stop_hook_active stays true", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-ultrawork-repeat-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
	...
	it("re-blocks active ralplan skill state on repeated Stop hooks", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-skill-repeat-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			...
			assert.equal(repeated.outputJson?.decision, "block");
			assert.match(
				String(repeated.outputJson?.reason ?? ""),
				/Status: continue_from_artifact/,
			);
			assert.match(
				String(repeated.outputJson?.reason ?? ""),
	...
	it("does not block Stop from another session's stale root team state when no scoped team state exists", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-stale-root-team-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
	...
	it("does not block Stop from orphaned team mode state after cleanup removed canonical team artifacts", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-orphaned-team-state-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
	...
	it("prefers the current session team state over a stale root team fallback during Stop", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-current-session-team-preferred-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "sessions", "sess-current", "team-state.json"),
				{
					active: true,
					current_phase: "starting",
					team_name: "current-team",
	...
	it("does not fall back to active root team state when the current scoped team state is inactive", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-native-hook-stop-inactive-scoped-team-"),
		);
		try {
			const stateDir = join(cwd, ".omx", "state");
			await mkdir(join(stateDir, "sessions", "sess-current"), {
			...
			await writeJson(
				join(stateDir, "team", "root-fallback-team", "phase.json"),
				{
					current_phase: "team-exec",
					max_fix_attempts: 3,
					current_fix_attempt: 0,
