...
describe("codex native hook dispatch", () => {
  ...
  it("ignores native hook routing text when detecting prompt keywords", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omx-native-hook-routing-text-"));
    try {
      await mkdir(join(cwd, ".omx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook({
        hook_event_name: "UserPromptSubmit",
        cwd,
        session_id: "sess-routing-text-1",
        thread_id: "thread-routing-text-1",
        turn_id: "turn-routing-text-1",
        prompt: "OMX ultraqa is still active (phase: planning); continue the task and gather fresh verification evidence before stopping.\n이거 왜 매번 훅이 걸리는거야?",
      }, { cwd });
  ...
  it("still records explicit ultraqa user requests", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "omx-native-hook-explicit-ultraqa-"));
    try {
      await mkdir(join(cwd, ".omx", "state"), { recursive: true });
      const result = await dispatchCodexNativeHook({
        hook_event_name: "UserPromptSubmit",
        cwd,
        session_id: "sess-explicit-ultraqa-1",
        thread_id: "thread-explicit-ultraqa-1",
        turn_id: "turn-explicit-ultraqa-1",
        prompt: "ultraqa 하자",
      }, { cwd });

      assert.equal(result.omxEventName, "keyword-detector");
      assert.equal(result.skillState?.skill, "ultraqa");
      assert.ok(result.outputJson, "explicit ultraqa should still emit developer context");
      const state = JSON.parse(await readFile(join(cwd, ".omx", "state", "skill-active-state.json"), "utf-8")) as Record<string, unknown>;
      assert.equal(state.skill, "ultraqa");
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
