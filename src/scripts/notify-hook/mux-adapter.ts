...
function metaFromPlainLine(line: string, env: NodeJS.ProcessEnv): CmuxSurfaceMeta | null {
  ...
  if (tabParts.length >= 2) {
    const [id = '', activeRaw = '', currentCommand = '', startCommand = '', currentPath = ''] = tabParts;
...
function finalizeCmuxRows(rows: CmuxSurfaceMeta[], env: NodeJS.ProcessEnv): CmuxSurfaceMeta[] {
  ...
  return rows.map((row, index) => {
    ...
    const inferredCurrentSurfaceAgent = active && currentSurface && row.id === currentSurface;
    return {
      ...row,
      active,
      currentCommand: row.currentCommand || (inferredCurrentSurfaceAgent ? 'codex' : ''),
      startCommand: row.startCommand || (inferredCurrentSurfaceAgent ? 'codex' : ''),
...
function normalizeCmuxPaneInModeStdout(invocation: MuxInvocation, stdout: string, env: NodeJS.ProcessEnv): string {
  ...
  const targetLines = target
    ? lines.filter((line) => line.includes(target))
    : lines;
  const inspected = targetLines.length > 0 ? targetLines : lines;
