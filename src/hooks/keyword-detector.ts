...
const KEYWORDS_REQUIRING_INTENT = new Set(['ralph', 'team', 'runningteam', 'stop', 'abort', 'parallel', 'autoresearch', 'ultragoal']);

type IntentKeyword = 'ralph' | 'team' | 'runningteam' | 'stop' | 'abort' | 'parallel' | 'autoresearch' | 'ultragoal';
...
const KEYWORD_INTENT_PATTERNS: Record<IntentKeyword, RegExp[]> = {
  ...
  runningteam: [
    /(?:^|[^\w])\$(?:runningteam)\b/i,
    /\/prompts:runningteam\b/i,
    /\b(?:use|run|start|enable|launch|invoke|activate|orchestrate|coordinate)\s+(?:a\s+|an\s+|the\s+)?runningteam\b/i,
    /\brunningteam\s+(?:mode|orchestration|workflow|skill|loop)\b/i,
  ],
  stop: [
