...
describe("keyword detector swarm/team compatibility", () => {
	it("keeps explicit $skill order in detectKeywords results (left-to-right)", () => {
		...
		assert.deepEqual(matches.map((m) => m.skill).slice(0, 3), [
			"analyze",
			"ultraqa",
			"code-review",
		]);
	...
	it("does not auto-detect keywords for explicit /prompts invocation without $skills", () => {
		...
		const primary = detectPrimaryKeyword(
			"/prompts:architect analyze this issue",
		);
	...
	it("supports explicit multi-skill invocation by prioritizing left-most $skill", () => {
		const match = detectPrimaryKeyword(
			"$ultraqa $analyze $code-review run now",
		);
		assert.ok(match);
	...
	it('maps "coordinated team" phrase to team orchestration skill', () => {
		const match = detectPrimaryKeyword(
			"run a coordinated team for implementation",
		);
	...
	it('maps "coordinated swarm" phrase to team orchestration skill', () => {
		const match = detectPrimaryKeyword(
			"run a coordinated swarm for implementation",
		);
	...
	it("keeps swarm trigger priority aligned with team trigger", () => {
		const teamMatch = detectKeywords("use team agents for this").find(
			(entry) => entry.skill === "team",
		);
		const swarmMatch = detectKeywords("use swarm for this").find(
			(entry) => entry.skill === "team",
		);
	...
	it("does not trigger team keyword from filesystem/team-state path text", () => {
		const match = detectPrimaryKeyword(
			"You have 1 new message(s). Check .omx/state/team/execute-plan/mailbox/worker-3.json, act now, and reply with concrete progress (not ACK-only).",
		);
	...
	it("does not trigger team skill from incidental prose usage", () => {
		const match = detectPrimaryKeyword(
			"the team reviewed the document and shared feedback",
		);
		assert.equal(match, null);
	});
	...
	it("applies longest-match tie-breaker when priorities are equal", () => {
		const match = detectPrimaryKeyword(
			"please run a coordinated swarm for this",
		);
	...
	it('maps "deep interview" phrase to deep-interview skill', () => {
		const match = detectPrimaryKeyword(
			"please run a deep interview before planning",
		);
	...
	it('maps "interview me" to deep-interview skill', () => {
		const match = detectPrimaryKeyword(
			"interview me before we start implementation",
		);
...
describe("keyword registry coverage", () => {
	it("includes key team/swarm aliases in runtime keyword registry", () => {
		const registryKeywords = new Set(
			KEYWORD_TRIGGER_DEFINITIONS.map((v) => v.keyword.toLowerCase()),
		);
...
describe("keyword detector skill-active-state lifecycle", () => {
	...
	it("acquires a deep-interview input lock immediately on activation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
			...
			assert.deepEqual(result.input_lock?.blocked_inputs, [
				...DEEP_INTERVIEW_BLOCKED_APPROVAL_INPUTS,
			]);
			assert.equal(
				result.input_lock?.blocked_inputs.includes("next i should"),
				true,
			);
			assert.equal(
				result.input_lock?.message,
				DEEP_INTERVIEW_INPUT_LOCK_MESSAGE,
			);
		} finally {
			await rm(cwd, { recursive: true, force: true });
	...
	it("releases the deep-interview input lock on abort via cancel keyword", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-deep-interview-abort-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		try {
			await mkdir(stateDir, { recursive: true });
	...
	it("emits a warning when skill-active-state persistence fails", async () => {
		...
		assert.match(
			String(warnings[0][0]),
			/failed to persist keyword activation state/,
		);
	});

	it("preserves activated_at for same-skill continuation", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-continuation-"),
		);
		const stateDir = join(cwd, ".omx", "state");
	...
	it("resets activated_at when skill changes", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-skill-switch-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
		try {
	...
	it("resets activated_at when keyword changes within the same skill", async () => {
		const cwd = await mkdtemp(
			join(tmpdir(), "omx-keyword-state-keyword-switch-"),
		);
		const stateDir = join(cwd, ".omx", "state");
		const statePath = join(stateDir, SKILL_ACTIVE_STATE_FILE);
		try {
...
describe("isUnderspecifiedForExecution", () => {
	...
	it("flags prompt with only keyword and generic words", () => {
		assert.equal(
			isUnderspecifiedForExecution("team improve performance"),
			true,
		);
	});
	...
	it("passes prompt with a file path reference", () => {
		assert.equal(
			isUnderspecifiedForExecution("ralph fix src/hooks/bridge.ts"),
			false,
		);
	});
	...
	it("passes prompt with a directory/file path", () => {
		assert.equal(
			isUnderspecifiedForExecution("update src/hooks/emulator.ts"),
			false,
		);
	});
	...
	it("passes prompt with a camelCase symbol", () => {
		assert.equal(
			isUnderspecifiedForExecution("team fix processKeywordDetector"),
			false,
		);
	});
	...
	it("passes prompt with snake_case symbol", () => {
		assert.equal(
			isUnderspecifiedForExecution("fix user_model validation"),
			false,
		);
	});
	...
	it("passes prompt with an issue number", () => {
		assert.equal(
			isUnderspecifiedForExecution("autopilot implement #42"),
			false,
		);
	});
	...
	it("passes prompt with acceptance criteria keyword", () => {
		assert.equal(
			isUnderspecifiedForExecution(
				"add login - acceptance criteria: user sees error on bad password",
			),
			false,
		);
	});
	...
	it("passes prompt with a specific error reference", () => {
		assert.equal(
			isUnderspecifiedForExecution("ralph fix TypeError in auth handler"),
			false,
		);
	});
	...
	it("passes with force: escape hatch prefix", () => {
		assert.equal(
			isUnderspecifiedForExecution("force: ralph refactor the auth module"),
			false,
		);
	});
	...
	it("passes with ! escape hatch prefix", () => {
		assert.equal(
			isUnderspecifiedForExecution("! autopilot optimize everything"),
			false,
		);
	});
	...
	it("passes longer prompt that exceeds word threshold", () => {
		...
		const longVague =
			"please help me improve the overall quality and performance and reliability of this system going forward";
		assert.equal(isUnderspecifiedForExecution(longVague), false);
	});

	it("false positive prevention: camelCase identifiers pass", () => {
		assert.equal(
			isUnderspecifiedForExecution("fix getUserById to handle null"),
			false,
		);
	});
});
...
describe("applyRalplanGate", () => {
	...
	it("does not gate well-specified prompts", () => {
		const result = applyRalplanGate(
			["ralph"],
			"ralph fix src/hooks/bridge.ts null check",
		);
