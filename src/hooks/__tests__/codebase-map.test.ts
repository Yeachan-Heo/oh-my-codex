...
describe("generateCodebaseMap", () => {
	...
	it("lists tracked TypeScript files grouped by directory", async () => {
		...
		await writeFile(
			join(tempDir, "src", "hooks", "session.ts"),
			"export function startSession() {}",
		);
		await writeFile(
			join(tempDir, "src", "hooks", "overlay.ts"),
	...
	it("expands src/ into sub-directories", async () => {
		...
		await writeFile(
			join(tempDir, "src", "cli", "index.ts"),
			"export function main() {}",
		);
		await writeFile(
			join(tempDir, "src", "mcp", "state-server.ts"),
	...
	it("includes non-src top-level directories", async () => {
		...
		await writeFile(
			join(tempDir, "scripts", "notify-hook.js"),
	...
	it("does not include untracked files (security: no filename leakage)", async () => {
		...
		try {
			...
			await writeFile(
				join(secDir, "src", "secret-wip.ts"),
				"export function secretWork() {}",
			);

			const map = await generateCodebaseMap(secDir);
			assert.ok(map.includes("tracked"), "should include tracked file");
			assert.ok(
				!map.includes("secret-wip"),
				"must not expose untracked filename",
			);
		} finally {
...
describe("generateCodebaseMap integration with generateOverlay", () => {
	...
	beforeAll(async () => {
		...
		await mkdir(join(tempDir, "src", "hooks"), { recursive: true });
		await writeFile(
			join(tempDir, "src", "hooks", "my-module.ts"),
			"export function myFn() {}",
		);
		execSync("git add src/hooks/my-module.ts", {
			cwd: tempDir,
			stdio: "ignore",
		});
	});
	...
	it("injects codebase map section into overlay", async () => {
		...
		assert.ok(
			overlay.includes("Codebase Map"),
			"overlay should contain Codebase Map section",
		);
		assert.ok(
			overlay.includes("src/hooks"),
			"overlay should include src/hooks from map",
		);
		assert.ok(
			overlay.includes("my-module"),
			"overlay should include module name",
		);
	});
