...
import {
  ...
  CMUX_METADATA_UNAVAILABLE_WINDOW_ID,
...
describe('mux adapter selection', () => {
  ...
  it('translates the tmux injection argv subset to cmux argv', () => {
    ...
    assert.deepEqual(
      translateTmuxArgvForCmux(
        ['display-message', '-p', '-t', 'surface:abc', '#{window_id}'],
        { OMX_MUX: 'cmux', CMUX_WORKSPACE_ID: 'workspace:abc' },
      ),
      ['identify', '--workspace', 'workspace:abc', '--surface', 'surface:abc'],
    );
    assert.deepEqual(
      translateTmuxArgvForCmux(
        ['display-message', '-p', '-t', 'surface:abc', '#S'],
        { OMX_MUX: 'cmux', CMUX_WORKSPACE_ID: 'workspace:abc' },
      ),
      ['identify', '--workspace', 'workspace:abc', '--surface', 'surface:abc'],
    );
  ...
  it('does not fabricate codex command metadata for active cmux surfaces', () => {
    const env = {
      OMX_MUX: 'cmux',
      CMUX_SURFACE_ID: 'surface:current',
      CMUX_WORKSPACE_ID: 'workspace:abc',
    ...
    assert.equal(
      normalizeMuxStdout(invocation, 'surface:current\n', env),
      'surface:current\t1\t\t\n',
    );
  });
  ...
  it('filters cmux surface-health rows by exact surface identifier', () => {
    const env = {
      OMX_MUX: 'cmux',
    ...
    const invocation = resolveMuxInvocation(
      'tmux',
      ['display-message', '-p', '-t', 'surface:1', '#{pane_in_mode}'],
      env,
    );

    assert.equal(
      normalizeMuxStdout(invocation, 'surface:10 scroll_active=true\nsurface:1 scroll_active=false\n', env),
      '0\n',
    );
    assert.equal(
      normalizeMuxStdout(invocation, 'surface:10 scroll_active=false\nsurface:1 scroll_active=true\n', env),
      '1\n',
    );
  });
  ...
  it('normalizes target-aware cmux display metadata probes', () => {
    ...
    const windowId = resolveMuxInvocation(
      'tmux',
      ['display-message', '-p', '-t', 'surface:abc', '#{window_id}'],
      env,
    );
    const sessionName = resolveMuxInvocation(
      'tmux',
      ['display-message', '-p', '-t', 'surface:abc', '#S'],
      env,
    );
    ...
    assert.deepEqual(sessionName.args, ['identify', '--workspace', 'workspace:abc', '--surface', 'surface:abc']);
    ...
    assert.equal(normalizeMuxStdout(sessionName, metadata, env), 'workspace:target\n');
    assert.equal(normalizeMuxStdout(windowId, metadata, env), '@42\n');
  });

  it('normalizes cmux #S from the requested surface instead of the focused surface', () => {
    const env = {
      OMX_MUX: 'cmux',
      CMUX_SURFACE_ID: 'surface:focused',
      CMUX_WORKSPACE_ID: 'workspace:focused',
    };
    const sessionName = resolveMuxInvocation(
      'tmux',
      ['display-message', '-p', '-t', 'surface:target', '#S'],
      env,
    );
    const metadata = JSON.stringify({
      caller: {
        surface_ref: 'surface:target',
        workspace_ref: 'workspace:target',
      },
      focused: {
    ...
    assert.deepEqual(sessionName.args, ['identify', '--workspace', 'workspace:focused', '--surface', 'surface:target']);
    assert.equal(normalizeMuxStdout(sessionName, metadata, env), 'workspace:target\n');
  });
  ...
  it('returns conservative metadata sentinels when cmux display metadata is unavailable', () => {
    ...
    assert.equal(normalizeMuxStdout(sessionName, '', env), `${CMUX_METADATA_UNAVAILABLE_SESSION}\n`);
    assert.equal(normalizeMuxStdout(windowId, '', env), `${CMUX_METADATA_UNAVAILABLE_WINDOW_ID}\n`);
  });
