...
Runtime workflows such as `autopilot`, `ralph`, `ultrawork`, `ultraqa`, `team`/`swarm`, and `ecomode` require OMX CLI runtime support. In Codex App, outside-tmux, or plain Codex sessions without an attached tmux runtime, continue with the nearest App-safe surface unless an explicit supported runtime is available. On Windows/Codex App, prompt-side `$team` may activate and should launch durable workers from shell with `$env:OMX_TEAM_RUNTIME='win-psmux'; omx team ...`; otherwise use an attached tmux shell for team/swarm runtime execution.
