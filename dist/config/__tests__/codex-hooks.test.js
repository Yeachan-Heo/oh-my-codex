...
import { buildManagedCodexNativeHookCommand, buildManagedCodexNativeHookWindowsShimContent, buildManagedCodexNativeHookWindowsShimPath, buildManagedCodexHookTrustState, buildManagedCodexHookTrustToml, buildManagedCodexHooksConfig, discoverCodexHookConfigPaths, dedupeCodexHookConfigPaths, getMissingManagedCodexHookEvents, hasUserCodexHooksAfterManagedRemoval, isRuntimeCodexHomeMirrorPath, mergeManagedCodexHooksConfig, removeManagedCodexHooks, } from "../codex-hooks.js";
describe("codex hooks helpers", () => {
    ...
    it("registers SessionStart for startup, resume, and clear reset sources", () => {
        ...
        assert.match(sessionStart?.matcher ?? "", /(?:^|\|)clear(?:\||$)/, "Codex emits SessionStart source=clear after /clear replacement threads; OMX must keep beginning-of-session hooks active");
        assert.match(sessionStart?.matcher ?? "", /(?:^|\|)startup(?:\||$)/, "fresh /new thread starts remain covered by Codex's startup SessionStart source");
    });
    it("uses a PowerShell ProcessStartInfo shim for Windows managed hook commands", () => {
        const config = buildManagedCodexHooksConfig("D:\\Program Files\\nvm\\v24.12.0\\node_modules\\oh-my-codex", { platform: "win32", codexHomeDir: "C:\\Users\\Ada Lovelace\\.codex" });
        const command = config.hooks.SessionStart[0]?.hooks?.[0]?.command;
        assert.equal(command, 'powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\Ada Lovelace\\.codex\\hooks\\omx-native-hook-windows-shim.ps1"');
    ...
    it("keeps Windows shim paths quoted when they contain spaces", () => {
        ...
        }), "D:\\Program Files\\nvm\\v24.12.0\\node_modules\\oh-my-codex", "C:\\Users\\Ada Lovelace\\.codex\\hooks.json", { platform: "win32", codexHomeDir: "C:\\Users\\Ada Lovelace\\.codex" }));
        const commands = merged.hooks.PreToolUse
            .flatMap((entry) => entry.hooks ?? [])
            .map((hook) => hook.command);
        assert.ok(commands.includes("echo keep-me"));
        assert.ok(commands.includes('powershell.exe -NoProfile -ExecutionPolicy Bypass -File "C:\\Users\\Ada Lovelace\\.codex\\hooks\\omx-native-hook-windows-shim.ps1"'));
    });
    it("builds deterministic Windows shim paths and PowerShell 5.1-compatible ProcessStartInfo content", () => {
        assert.equal(buildManagedCodexNativeHookWindowsShimPath("C:\\Users\\Ada Lovelace\\.codex"), "C:\\Users\\Ada Lovelace\\.codex\\hooks\\omx-native-hook-windows-shim.ps1");
        const content = buildManagedCodexNativeHookWindowsShimContent("D:\\Program Files\\O'Malley\\oh-my-codex", { nodePath: "C:\\Program Files\\nodejs\\node.exe" });
        ...
        assert.match(content, /\$startInfo\.Arguments = '"D:\\Program Files\\O''Malley\\oh-my-codex\\dist\\scripts\\codex-native-hook\.js"'/);
    });
    it("forwards payload, stdout, stderr, and non-zero exit through the Windows shim when PowerShell is available", async () => {
        ...
        try {
            ...
            await writeFile(shimPath, buildManagedCodexNativeHookWindowsShimContent(pkgRoot, {
                hookScriptPath: hookPath,
                nodePath: process.execPath,
            }));
            const result = spawnSync(shell, ["-NoProfile", "-ExecutionPolicy", "Bypass", "-File", shimPath], {
    ...
    it("matches Codex's normalized command hook hash identity", async () => {
        ...
        const command = `"${process.execPath.replace(/\\/g, "\\\\").replace(/"/g, '\\"')}" "/repo/dist/scripts/codex-native-hook.js"`;
    ...
    it("keeps hooks.json trust state outside the hook event map", () => {
        ...
        assert.ok(Object.values(merged.hooks).every(Array.isArray), "Codex Rust hook discovery expects hooks.hooks values to be event arrays");
        ...
        assert.match(merged.state["/hooks.json:stop:0:0"]?.trusted_hash ?? "", /^sha256:[a-f0-9]{64}$/);
    ...
    it("preserves top-level managed hook state metadata while updating trusted_hash", () => {
        ...
        const managedKey = Object.keys(managedState).find((key) => key.includes(":stop:")) ?? Object.keys(managedState)[0];
    ...
    it("keeps Windows shim hook merge idempotent while replacing stale direct-node wrappers", () => {
        ...
        const first = mergeManagedCodexHooksConfig(stale, "D:\\Program Files\\nvm\\v24.12.0\\node_modules\\oh-my-codex", "C:\\Users\\Ada Lovelace\\.codex\\hooks.json", options);
        const second = mergeManagedCodexHooksConfig(first, "D:\\Program Files\\nvm\\v24.12.0\\node_modules\\oh-my-codex", "C:\\Users\\Ada Lovelace\\.codex\\hooks.json", options);
        assert.equal(second, first);
    ...
    it("removes only OMX-managed wrappers during uninstall cleanup", () => {
        ...
        const generatedWithTrustState = mergeManagedCodexHooksConfig(null, "/repo", "/hooks.json");
        const removedGeneratedWithTrustState = removeManagedCodexHooks(generatedWithTrustState);
    ...
    it("registers managed compact hook wrappers", () => {
        ...
        const postCompact = config.hooks.PostCompact;
