...
- **ralplan**: Consensus planning (planner + architect + critic). The stage is complete only when the Planner output has been reviewed sequentially by Architect and then Critic, with both review artifacts/verdicts recorded. It skips only when both `prd-*.md` and `test-spec-*.md` planning artifacts already exist **and** prior Architect/Critic consensus evidence is available, and carries any `deep-interview-*.md` spec paths forward for traceability.
