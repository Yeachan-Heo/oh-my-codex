...
**Native role-routing preflight:** Before adapted planner/reviewer delegation, consensus-release, or any other Ralplan-originated action requiring adapted role authority, inspect the native task surface. Keyword routing may already have selected Ralplan, but it is not authority. When the surface exposes `agent_type`, use typed native routing and do not run the adapted preflight. When it reports `role_routing_unavailable`, run `omx ralplan preflight --json`; on `unsupported_documented_leader_proof`, stop before adapted planner/reviewer work, adapted delegation, or consensus-release authority and use a Codex surface with documented root proof or a reviewed alternative workflow. Do not apply this gate to unrelated guarded install/sync, status/health checks, or runtime work; those flows retain their own safety gates. Do not infer root identity from `session_id`, undocumented `thread_id`, session/pointer/transcript/cwd state, absence of child data, or a prompt label.
