...
- **On ralplan -> ultragoal**: after Architect→Critic lifecycle consensus is complete, authorize the transition by persisting `ralplan_execution_handoff:{authorized_by_user:true, reason:"<rationale>", authorized_at:"<timestamp>", session_id:"<session>", review_cycle:<cycle>, source:"user"}`. This user-authorized handoff is the reachable execution path (distinct from host-consensus authority). The host-receipt verifier path remains separate and unavailable. Without either authorization, keep `current_phase:"ralplan"` and persist `blocked_reason:"documented_host_consensus_receipt_unavailable"`. Locally authored JSON/env/prompt/tracker/transcript/receipt-shaped evidence remains non-authorizing for the host-consensus gate.
  - The stable `<!-- OMX:AUTOPILOT:RALPLAN-ULTRAGOAL-HANDOFF:v1 -->` marker immediately precedes the executable handoff fence; automation may locate exactly that fence.

<!-- OMX:AUTOPILOT:RALPLAN-ULTRAGOAL-HANDOFF:v1 -->
```bash
omx state write --input '{"mode":"autopilot","active":true,"current_phase":"ultragoal","session_id":"'"${OMX_SESSION_ID:?authoritative OMX session required}"'","workingDirectory":"'"${PWD:?working directory required}"'","state":{"ralplan_execution_handoff":{"authorized_by_user":true,"reason":"user reviewed the Architect→Critic consensus plan and authorized execution","authorized_at":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","session_id":"'"${OMX_SESSION_ID}"'","review_cycle":'"${REVIEW_CYCLE:?current ralplan review_cycle required}"'","source":"user"}}}' --json
```
  - `OMX_SESSION_ID` must equal the session that ran the ralplan consensus. `REVIEW_CYCLE` must equal the current ralplan `review_cycle` (the cycle on which Architect and Critic both approved). The gate validates both against the lifecycle evidence and rejects cross-session or stale-cycle handoffs.
