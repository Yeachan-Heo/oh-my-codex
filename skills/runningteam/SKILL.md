...
RunningTeam is the dynamic planning workflow for OMX. Use it when the user wants `ralplan`-quality planning and `team`-style parallel execution without manually running `$ralplan` and then `$team`.

## Core Contract

RunningTeam is not a static pre-plan. It is a checkpoint-gated controller:
...
```bash
omx --runningteam --madmax "ship the feature"
```

Alternative command form:

```bash
omx runningteam "ship the feature"
```

Prompt-side activation:

```text
$runningteam ship the feature
```

## Behavior

When RunningTeam is active:
...
6. Keep state in existing OMX state surfaces and RunningTeam controller state.
7. Require final synthesis and verification evidence before completion.

## Safety Rules

- Do not require a separate `$ralplan` first; RunningTeam owns dynamic planning.
