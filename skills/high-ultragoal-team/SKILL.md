...
Non-overlap: use `$ultragoal` for goal ledgers alone, `$team` for worker runtime alone, and `$ralplan` for planning alone. Use this recipe only for the combined leader-owned Ultragoal + Team operating contract.
...
- Workers implement, inspect, test, review, and report verifiable artifacts only.
...
- After `update_goal({status:"complete"})`, treat the thread goal as terminal for this recipe. Before unrelated new work in the same Codex session/thread, the operator must manually clear the completed goal with `/goal clear` or explicitly move to another leader context; hooks and shell commands must not clear hidden Codex goal state.
...
- `omx team`: durable parallel worker lanes and verifiable artifact reporting.
- `omx ralph` / `$ralph`: optional serial follow-up when Team artifacts show a blocker is no longer parallelizable. Do not use removed `omx team ralph`.
...
- If `get_goal` reports a completed unrelated/legacy goal in this thread, do not start a new unrelated campaign yet. Record a non-terminal Ultragoal blocker if needed, then continue only after the operator manually runs `/goal clear` and a fresh `get_goal` confirms no conflicting goal, or after the operator explicitly chooses a separate leader context.
...
```bash
omx ultragoal create-goals --brief "<scoped brief>" [--force]
```

- For a new wave inside an active campaign, prefer pending goals or add a scoped goal:

```bash
omx ultragoal add-goal --title "<wave>" --objective "<scoped objective>"
```
...
3. Launch 0-2 high workers only if parallel artifact collection helps.
...
3. Run each wave through Team artifacts, leader integration, verification, and checkpoint.
...
omx team 3:executor "Work on the active Ultragoal story. Do not mutate .omx/ultragoal or create/update Codex goals. Report files changed, tests run, verifiable artifacts, risks, and whether prohibited state was touched."
```
...
Workers produce verifiable artifacts, not authority. Before accepting worker output, the leader must:
...
If `.omx/ultragoal` has an in-progress story but `get_goal` returns a completed unrelated/legacy objective from an earlier run in the same thread:
...
6. Continue only after the operator manually clears the completed Codex goal with `/goal clear` and a fresh `get_goal` confirms no conflicting goal, or after the operator explicitly chooses a separate leader context in the same repo/worktree.
...
|---|---|---|
| `quick-wave` | Targeted verification, leader diff review, mutation guard, concise Ultragoal checkpoint if an Ultragoal story is active | Public API, security, migrations, broad refactor, worker conflict, or suspicious/low-artifact output |
| `campaign` | Per-wave targeted verification, mutation guard, checkpoint with artifacts/evidence; final campaign gate before Codex goal completion | Review blockers, cross-wave coupling, flaky tests, or non-trivial architecture changes |
| `deliberate-campaign` | Approved `ralplan --deliberate` acceptance criteria, per-wave verification/checkpoints, final gate with review artifacts | Any plan drift, principle violation, security/compliance concern, or unresolved architect/critic issue |
...
- The leader's Codex goal is already `complete` and the work is unrelated; record a blocked Ultragoal checkpoint if an in-progress story was created, then stop until `/goal clear` is manually run or the operator explicitly chooses a separate leader context.
...
- Worker output lacks verifiable artifacts or touches prohibited `.omx/ultragoal` state.
