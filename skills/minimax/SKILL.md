---
name: minimax
description: "[OMX] One-step-ahead adversarial oversight workflow. Use when the user explicitly asks for `$minimax` or a minimax workflow to run a proposer, lookahead reviewer, arbiter loop, with optional independent review escalation for risky diffs."
---
...
- Independent review should run only on high-risk or disputed steps.
...
- **COUNCIL**: optional high-cost independent review. Use an available review surface that can produce a durable, scoped artifact; do not bake any specific tool, vendor, or integration into Minimax.

The same model context may perform multiple roles for small tasks, but keep the role outputs separate. For meaningful code changes, use role-specific native subagents when available: an implementer for MAX, a critic for MIN, and a verifier or reviewer for ARBITER/COUNCIL.
...
For public API, security, package-export, or compatibility-contract changes, require a durable independent-review artifact before final completion unless no review surface is available and the fallback is recorded. The artifact must name the review scope, inputs, reviewer surface, verdict, blocker count, degraded/partial-coverage status, and source evidence. Do not rely on a single prose note, scalar verdict, or assistant-authored summary without underlying review artifacts.

Minimal review artifact shape:

```json
{
  "schema_version": "minimax-independent-review-v1",
  "reviewer_surface": "<independent reviewer or review workflow>",
  "scope": "<what was reviewed>",
  "inputs": ["<file/range/log/artifact>"],
  "verdict": {"status": "pass|no_blockers|block|uncertain", "rationale": "..."},
  "blockers": [],
  "blocker_count": 0,
  "degraded": false
}
...
When an independent review is required, use the narrowest available review surface that can inspect the relevant evidence and write a durable artifact. Include only files, ranges, logs, and context needed for the claim. Treat review findings as claims to verify, not as automatic truth. If no independent review surface is available, record that limitation and use the strongest local verification you can run.
