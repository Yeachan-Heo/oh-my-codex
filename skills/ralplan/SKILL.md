...
**Adapted role-intent step:** On a `role_routing_unavailable` surface, before each App Architect or Critic spawn, run `omx ralplan role-intent write --role <role> --parent-thread <leader-thread-id> --json`; read `intent.correlation_token` from its receipt, then spawn the App Architect or Critic with that exact token as `agent_nickname`. The recorded validated intent and correlation token are the authoritative role carrier, never a prompt label.
