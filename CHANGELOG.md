...
- **Ralplan authority policy** — this fork compiles `authority_policy: "local_owner_lifecycle"`. A fresh, approving, tracker-backed native Architect review followed by a fresh, approving, tracker-backed native Critic review, with distinct completed native thread identities, authorizes Ralplan handoff. The resulting local lifecycle authority is never a `host_consensus_receipt`; adapted role-intent restrictions remain unchanged.
...
> **Current status / supersession:** The 0.20.3 entries above remain historical. This fork now compiles `authority_policy: "local_owner_lifecycle"`: a fresh, approving, tracker-backed native Architect review followed by a fresh, approving, tracker-backed native Critic review, with distinct completed native thread identities, authorizes Ralplan handoff. This local lifecycle authority is never a `host_consensus_receipt`. The former receipt-only policy reported `documented_host_consensus_receipt_unavailable`; that blocker is historical and does not describe the current compiled policy. Local leader attestation and adapted role intent still do not authorize, and `role_routing_unavailable` adapted authority attempts still fail with `unsupported_documented_leader_proof`.
...
> **Current status / supersession:** The 0.20.2 entries above remain historical. This fork now compiles `authority_policy: "local_owner_lifecycle"`: a fresh, approving, tracker-backed native Architect review followed by a fresh, approving, tracker-backed native Critic review, with distinct completed native thread identities, authorizes Ralplan handoff. This local lifecycle authority is never a `host_consensus_receipt`. The former receipt-only policy reported `documented_host_consensus_receipt_unavailable`; that blocker is historical and does not describe the current compiled policy. Local leader attestation and adapted role intent still do not authorize, and `role_routing_unavailable` adapted authority attempts still fail with `unsupported_documented_leader_proof`.
