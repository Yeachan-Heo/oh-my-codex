...
This fork compiles Ralplan consensus with `authority_policy: "local_owner_lifecycle"`. The gate authorizes Ralplan handoff only after it validates a fresh, approving, tracker-backed native Architect review followed by a fresh, approving, tracker-backed native Critic review for the current planning pass. The two completed reviews must have native-subagent provenance and distinct native thread identities, and Architect must precede Critic. Tracker records alone, locally authored gate fields, adapted-role evidence, same-thread reviews, reversed order, stale reviews, and non-approving reviews remain insufficient.

A successful `local_owner_lifecycle` decision is local lifecycle authority. It is never a `host_consensus_receipt` and must not be described, serialized, or reported as one. The compact-JWS host receipt work remains an optional stronger future policy, not the active policy and not a label for tracker-backed approval. The packaged plugin's `OMX_CODEX_LAUNCH_ID` and `plugin-hook-routing` record remain spoofable routing-only discriminators, not secrets, signed claims, or authority sources. See [ADR 3194](./adr/3194-codex-01445-documented-leader-proof.md), [ADR 3212](./adr/3212-same-user-native-child-auth-boundary.md), the [host receipt Phase 1 ADR](./adr/codex-host-consensus-receipt-phase-1.md), and the [consensus gate contract](./contracts/ralplan-consensus-gate.md).
