...
| `display-message -p ... '#{pane_in_mode}'` | `surface-health --workspace <workspace>`, normalized to `0`/`1` |
| `display-message -p ... '#{pane_id}'` | `display-message -p <surface>` |
| `display-message -p ... '#{pane_current_command}'` | `identify --workspace <workspace> --surface <surface>`, normalized to the target command |
| `display-message -p ... '#{pane_start_command}'` | `identify --workspace <workspace> --surface <surface>`, normalized to the target start command or a fail-closed sentinel |
| `display-message -p ... '#{pane_current_path}'` | `identify --workspace <workspace> --surface <surface>`, normalized to the target cwd or a fail-closed sentinel |
| `display-message -p ... '#{window_id}'` | `identify --workspace <workspace> --surface <surface>`, normalized to the target window id or a fail-closed sentinel |
| `display-message -p -t <surface> '#S'` | `identify --workspace <workspace> --surface <surface>`, normalized to the target workspace/session or a fail-closed sentinel |
| `list-panes -t <workspace> -F <format>` | `list-pane-surfaces --workspace <workspace>`, normalized back to the requested tmux `-F` fields |
