...
8. `deep-interview -> ralplan` is evidence-gated: answered or handoff-cleared question obligations alone do not complete deep-interview. Autopilot `ralplan -> ultragoal` uses the compiled `authority_policy: "local_owner_lifecycle"` and is authorized only by fresh, approving, tracker-backed native Architect then Critic reviews from distinct completed native threads for the current planning pass. The resulting local lifecycle authority is never a `host_consensus_receipt`; tracker records alone, `codex_exec`, locally authored gate fields, adapted-role evidence, stale reviews, same-thread reviews, reversed order, and non-approving reviews remain non-authoritative.
